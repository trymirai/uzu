use std::{collections::HashSet, env, fs, path::{Path, PathBuf}, process::Command, time::SystemTime};
use std::io::Write;
...
fn compile_metal_shaders() {
    ...
    if !Command::new("xcrun").arg("--version").status().map(|s| s.success()).unwrap_or(false) {
        panic!("xcrun not found. Please install Xcode command-line tools to compile Metal shaders.");
    }

    // Platform-specific metallib path (separate for macos vs ios for caching)
    let platform_suffix = if sdk == "macosx" || sdk == "maccatalyst" { "macos" } else { "ios" };
    let metallib_path = metallib_dir.join(format!("shaders_{}.metallib", platform_suffix));

    // Decide whether to compile (if metallib missing or any source changed)
    let should_compile = needs_compilation(&metal_files, &metallib_path, &target);
    if should_compile {
        ...
        println!("cargo:info=Metal shaders are up-to-date; skipping compilation");
    }
...
fn needs_compilation(metal_files: &[PathBuf], metallib_path: &Path, target: &str) -> bool {
    // If output metallib doesn't exist, need to compile
    if !metallib_path.exists() {
        return true;
    }
    // If the previously built target differs (target changed, e.g. arch or OS)
    if let Ok(prev_target) = fs::read_to_string(metallib_path.with_extension("target")) {
        if prev_target != target {
            println!("cargo:warning=Target changed from {} to {}, recompiling shaders", prev_target, target);
            return true;
        }
    }
    // If any source file is newer than the metallib, recompile
    let metallib_mtime = fs::metadata(metallib_path).and_then(|m| m.modified()).unwrap_or(SystemTime::UNIX_EPOCH);
    for metal_file in metal_files {
        if let Ok(src_mtime) = fs::metadata(metal_file).and_then(|m| m.modified()) {
...
fn compile_metal_files(metal_files: &[PathBuf], out_dir: &Path, sdk: &str, src_dir: &Path) {
    ...
    for metal_file in metal_files {
        ...
        let prefix = if sdk == "macosx" || sdk == "maccatalyst" { "macos_" } else { "ios_" };
        ...
        for flag in &platform_flags { cmd.arg(flag); }
        ...
        cmd.args(&["-c", metal_file.to_str().unwrap(), "-o", air_path.to_str().unwrap()]);

        println!("cargo:info=Running Metal compiler: {}", cmd.get_args().map(|a| a.to_string_lossy()).collect::<Vec<_>>().join(" "));
    ...
    let metallib_path = out_dir.join(if sdk == "macosx" || sdk == "maccatalyst" { "shaders_macos.metallib" } else { "shaders_ios.metallib" });
    ...
    println!("cargo:info=Linking Metal libraries: {}", lib_cmd.get_args().map(|a| a.to_string_lossy()).collect::<Vec<_>>().join(" "));
    ...
    println!("cargo:info=Metal shaders compiled successfully to {}", metallib_path.display());
}
