...
use crate::{
    DownloadError, DownloadId, FileCheck, FileDownloadManager, FileDownloadState, FileDownloadTask, LockFileInfo,
    LockFileState, compute_download_id,
    backends::{
        common::{
            Backend, DownloadManager, DownloadManagerState, InitialTaskAttachment, manager::observe_startup,
            manager::prepare_startup,
        },
        universal::{UniversalBackend, UniversalBackendContext, UniversalBackendError},
    },
    file_download_task::{CachedFileDownloadTask, ManagedFileDownloadTask},
...
impl Backend for FailingAttachmentBackend {
    ...
    fn create_context(_tokio_handle: tokio::runtime::Handle) -> Result<FailingAttachmentContext, DownloadError> {
...
async fn prepare_startup_reobserves_lock_before_cleanup_actions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (reobserved_startup, startup_lease) = prepare_startup::<UniversalBackend>(
        initial_startup,
        &UniversalBackendContext::new(tokio::runtime::Handle::current()),
        download_id,
        "http://example.invalid/model.bin",
        &destination,
        FileCheck::CRC("00000000".to_string()),
...
async fn remove_file_task_cleans_construction_lock_when_shutdown_fails() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let manager = DownloadManager::<UniversalBackend> {
        state: state.clone(),
        context: Arc::new(UniversalBackendContext::new(tokio::runtime::Handle::current())),
    };
