...
fn get_output<T: ArrayElement + Float, B: Backend>(input: &Input<T>) -> Vec<T> {
    ...
    let kernel = <<B as Backend>::Kernels as Kernels>::PoolingClsKernel::new(&context, T::data_type())
        .expect("Failed to create PoolingClsKernel");
...
fn test_internal<T: ArrayElement + Float + Debug + Display>(
    ...
    for_each_non_cpu_backend!(|B| {
        ...
        let msg = format!("PoolingCls test failed for backend {}", std::any::type_name::<B>());
