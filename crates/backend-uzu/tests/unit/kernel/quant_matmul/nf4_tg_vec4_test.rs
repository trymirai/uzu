//! Correctness gate for `Nf4QmvTgVec4` (4x replicated TG codebook
...
#[uzu_test]
fn nf4_tg_vec4_qmv_equivalent_to_constant() {
    ...
    for &(m, k, n) in &[(1usize, 512usize, 128usize), (2, 512, 128), (4, 512, 256), (2, 1024, 128)] {
        ...
        let rep = run_qmv(&ctx, &bench, Nf4Variant::TgVec4, &p);
        let (worst_rel, worst_abs) = worst_errors(&cst, &rep);
        eprintln!(
            "[nf4_tg_vec4_eq] M={} K={} N={} worst_rel={:.3e} worst_abs={:.3e}",
            m, k, n, worst_rel, worst_abs
        );
        assert!(
            worst_rel < 1e-3,
            "Nf4QmvTgVec4 not equivalent to Nf4QmvConstant (M={} K={} N={}): worst_rel={:.3e} worst_abs={:.3e}",
            m,
            k,
            n,
            worst_rel,
