//! Correctness gate for `Nf4QmvByte256Dup` (FLUTE-style: byte-batched
//! 256-entry threadgroup `half2` LUT replicated D ∈ {8, 16, 32} times in
//! threadgroup memory, lane-id picks the copy). Each replica slot holds the
//! same codebook pair, so the kernel must be bit-equivalent to
//! `Nf4QmvConstant` modulo the bf16 storage tail.
...
#[uzu_test]
fn nf4_byte256_dup_qmv_equivalent_to_constant() {
    let ctx = Ctx::new().expect("Metal context required");
    let device = {
        use metal::MTLDeviceExt;
        ctx.device.name()
    };
    eprintln!("[nf4_byte256_dup_eq] device={}", device);
    ...
    for &(m, k, n) in &[(1usize, 512usize, 128usize), (2, 512, 128), (4, 512, 256), (2, 1024, 128)] {
        ...
        for &variant in &[Nf4Variant::Byte256Dup8, Nf4Variant::Byte256Dup16, Nf4Variant::Byte256Dup32] {
            let dup = run_qmv(&ctx, &bench, variant, &p);
            let (worst_rel, worst_abs) = worst_errors(&cst, &dup);
            eprintln!(
                "[nf4_byte256_dup_eq] {:?} M={} K={} N={} worst_rel={:.3e} worst_abs={:.3e}",
                variant, m, k, n, worst_rel, worst_abs
            );
            // Same dequant math + identical LUT contents per replica; accept
            // up to one bf16 ULP of output slack.
            assert!(
                worst_rel < 2.5e-2,
                "{:?} not equivalent to Nf4QmvConstant (M={} K={} N={}): worst_rel={:.3e} worst_abs={:.3e}",
                variant,
                m,
                k,
                n,
