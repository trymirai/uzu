//! Correctness gate for `Nf4QmvTgSimdbar` (8-copy simdgroup-local TG
//! codebook ordered with `simdgroup_barrier(mem_threadgroup)`). Must be
//! numerically equivalent to the existing `Nf4QmvConstant` for identical
//! inputs: same dequant math, only the codebook origin/access ordering
//! differs (per-simdgroup TG-memory copy + simdgroup-scope barrier).
...
#[uzu_test]
fn nf4_tg_simdbar_qmv_equivalent_to_constant() {
    ...
    for &(m, k, n) in &[(1usize, 512usize, 128usize), (2, 512, 128), (4, 512, 256), (2, 1024, 128)] {
        ...
        let sbar = run_qmv(&ctx, &bench, Nf4Variant::TgSimdbar, &p);
        let (worst_rel, worst_abs) = worst_errors(&cst, &sbar);
        eprintln!(
            "[nf4_tg_simdbar_eq] M={} K={} N={} worst_rel={:.3e} worst_abs={:.3e}",
            m, k, n, worst_rel, worst_abs
        );
        assert!(
            worst_rel < 1e-3,
            "Nf4QmvTgSimdbar not equivalent to Nf4QmvConstant (M={} K={} N={}): worst_rel={:.3e} worst_abs={:.3e}",
            m,
            k,
            n,
            worst_rel,
