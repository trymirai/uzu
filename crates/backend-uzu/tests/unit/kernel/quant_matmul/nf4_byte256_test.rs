//! Correctness gate for `Nf4QmvByte256` (byte-batched 256-entry threadgroup
//! `half2` codebook LUT). It must be numerically equivalent to the existing
//! `Nf4QmvConstant` for identical NF4 inputs (same codebook + same packed
//! weights + same bf16 scales): same dequant math, only the codebook access
//! pattern differs (per-nibble `constant` load vs one byte → one threadgroup
//! `half2` load). Expect ~0 / tiny rel err.
...
#[uzu_test]
fn nf4_byte256_qmv_equivalent_to_constant() {
    let ctx = Ctx::new().expect("Metal context required");
    let device = {
        use metal::MTLDeviceExt;
        ctx.device.name()
    };
    eprintln!("[nf4_byte256_eq] device={}", device);
    ...
    for &(m, k, n) in &[(1usize, 512usize, 128usize), (2, 512, 128), (4, 512, 256), (2, 1024, 128)] {
        ...
        let b256 = run_qmv(&ctx, &bench, Nf4Variant::Byte256, &p);
        let (worst_rel, worst_abs) = worst_errors(&cst, &b256);
        eprintln!(
            "[nf4_byte256_eq] M={} K={} N={} worst_rel={:.3e} worst_abs={:.3e}",
            m, k, n, worst_rel, worst_abs
        );
        // Same dequant math, different codebook access — both produce bf16
        // outputs from the SAME fp32 accumulation order, so this must be
        // exact (or within a single bf16 ULP of the stored output).
        assert!(
            worst_rel < 1e-3,
            "Nf4QmvByte256 not equivalent to Nf4QmvConstant (M={} K={} N={}): worst_rel={:.3e} worst_abs={:.3e}",
            m,
            k,
            n,
            worst_rel,
