...
use super::{TreeVerifyCores, TreeVerifyEncodeArguments, TreeVerifyNewArguments};
use crate::{
    backends::{
...
#[uzu_bench]
fn bench_delta_net_tree_verify_core(c: &mut Criterion) {
    ...
    let core = TreeVerifyCores::new(arguments, context.as_ref()).unwrap();
    eprintln!("GDN tree core PSO construction: {:.2} ms", started.elapsed().as_secs_f64() * 1e3);
    let h0 = alloc_allocation_with_data::<Metal, f32>(&context, &vec![0.001; V_HEADS * HEAD_DIM * HEAD_DIM]);
    let mut group = c.benchmark_group("Metal/Kernel/GDNTreeVerify/Core");
    ...
    for tree_size in [32usize, 49, 64, 128] {
        ...
        let benchmark_path = format!("Metal/Kernel/GDNTreeVerify/Core/T{tree_size}");
