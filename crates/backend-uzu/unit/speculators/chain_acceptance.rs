...
fn test_full_chain_accepted_with_bonus() {
    ...
    assert_eq!(accepted.committed_token_ids, vec![11, 12, 13, 14]);
    assert_eq!(accepted.committed_token_positions, vec![6, 7, 8, 9]);
    assert_eq!(accepted.num_verified_nodes, 4);
    assert_eq!(accepted.next_context_nodes(), 0..4);
}
...
fn test_partial_chain_accepted() {
    ...
    assert_eq!(accepted.committed_token_ids, vec![11, 99]);
    assert_eq!(accepted.committed_token_positions, vec![6, 7]);
...
fn test_all_drafts_rejected_emits_bonus_only() {
    ...
    assert_eq!(accepted.committed_token_ids, vec![99]);
    assert_eq!(accepted.committed_token_positions, vec![6]);
    assert_eq!(accepted.num_verified_nodes, 1);
}
...
fn test_eos_at_first_accepted_draft() {
    ...
    assert_eq!(accepted.committed_token_ids, vec![11]);
...
fn test_root_only_proposal_emits_bonus() {
    ...
    assert_eq!(accepted.committed_token_ids, vec![42]);
    assert_eq!(accepted.committed_token_positions, vec![6]);
    assert_eq!(accepted.num_verified_nodes, 1);
}
