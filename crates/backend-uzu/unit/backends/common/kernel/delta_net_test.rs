    ...
    backends::{
        common::{
            ...
            kernel::{
                ...
                DeltaNetChunkedFusedApplyKernel, DeltaNetChunkedGramKernel, DeltaNetChunkedMegaApplyInlineKernel,
...
// fused path this is the chunked precompute chain (Prep, Cumsum, Gram, Solve,
...
// ===========================================================================
// DeltaNetChunkedMegaApplyInline (Mode S) correctness tests
// ===========================================================================

// Run the Mode S mega kernel on backend `B`. Unlike Mode L, it reads log_decay
// (not the precomputed g) and reconstructs gram / solve / A inline; it takes no
// qk_scaled or t_mat device buffers. VT=16 only (VT=32 exceeds the 32 KB TG cap).
#[allow(clippy::too_many_arguments)]
fn run_mega_apply_inline<B: Backend>(
    q_norm: &[f32],
    k_norm: &[f32],
    in_proj: &[bf16],
    log_decay: &[f32],
    ...
    let k_a = context.create_array_from(&[k_norm.len()], k_norm);
    let in_a = context.create_array_from(&[in_proj.len()], in_proj);
    let ld_a = context.create_array_from(&[log_decay.len()], log_decay);
    let beta_a = context.create_array_from(&[beta.len()], beta);
    let mut state_alloc = context.create_array_from(&[state.len()], state).into_allocation();
    let mut out_alloc = context.create_array_zeros(&[suffix_len * value_dim], DataType::F32).into_allocation();

    let kernel = <<B as Backend>::Kernels as Kernels>::DeltaNetChunkedMegaApplyInlineKernel::new(
        &context,
        DataType::BF16,
        DataType::F32,
        16,
    )
    .expect("mega inline kernel");
...
// Deterministic pseudo-random inputs for the Mode S kernel. Same q/k/in_proj/
// beta/state as make_mega_inputs, plus a per-token negative log_decay (the
// kernel forms the chunk-local prefix g itself).
#[allow(clippy::type_complexity)]
fn make_mega_inline_inputs(
    num_v_heads: usize,
    num_k_heads: usize,
    head_k_dim: usize,
    head_v_dim: usize,
    suffix_len: usize,
) -> (Vec<f32>, Vec<f32>, Vec<bf16>, Vec<f32>, Vec<f32>, Vec<f32>) {
    let key_dim = num_k_heads * head_k_dim;
    let value_dim = num_v_heads * head_v_dim;
    let conv_dim = 2 * key_dim + value_dim;
    let total_proj_dim = conv_dim + value_dim + num_v_heads + num_v_heads;
    ...
    let in_proj: Vec<bf16> =
        (0..suffix_len * total_proj_dim).map(|i| bf16::from_f32((((i * 5 + 1) % 29) as f32 - 14.0) * 0.01)).collect();
    // Per-token negative log-decay (its chunk-local prefix is a decaying g).
    let log_decay: Vec<f32> =
        (0..suffix_len * num_v_heads).map(|i| -(0.01 + (((i * 7 + 3) % 11) as f32) * 0.003)).collect();
    let beta: Vec<f32> = (0..suffix_len * num_v_heads).map(|i| 0.3 + (((i * 7 + 3) % 11) as f32) * 0.05).collect();
    let state: Vec<f32> = (0..state_size).map(|i| (((i * 19 + 4) % 29) as f32) * 0.004 - 0.05).collect();

    (q_norm, k_norm, in_proj, log_decay, beta, state)
}

// Test 1: Mode S Metal kernel vs its CPU mirror (identical random inputs). VT=16.
fn mega_inline_vs_cpu_mirror_impl(suffix_len: usize) {
    let num_v_heads = 48usize;
    let num_k_heads = 16usize;
    let head_k_dim = 128usize;
    let head_v_dim = 128usize;
    ...
    let (q_norm, k_norm, in_proj, log_decay, beta, state) =
        make_mega_inline_inputs(num_v_heads, num_k_heads, head_k_dim, head_v_dim, suffix_len);

    let (metal_out, metal_state) = run_mega_apply_inline::<Metal>(
        &q_norm,
        &k_norm,
        &in_proj,
        &log_decay,
    ...
    let (cpu_out, cpu_state) = run_mega_apply_inline::<Cpu>(
        &q_norm,
        &k_norm,
        &in_proj,
        &log_decay,
    ...
    let (state_abs, state_rel) = max_abs_rel_err(&metal_state, &cpu_state);
    eprintln!(
        "MEGA_INLINE_VS_MIRROR T={suffix_len}: out max_abs={out_abs:.3e} max_rel={out_rel:.3e} \
         state max_abs={state_abs:.3e} max_rel={state_rel:.3e}"
    );
    assert_close(&cpu_out, &metal_out, 5e-3, 5e-3, &format!("mega inline vs mirror out (T={suffix_len})"));
    assert_close(&cpu_state, &metal_state, 5e-3, 5e-3, &format!("mega inline vs mirror state (T={suffix_len})"));
}

#[uzu_test]
fn test_delta_net_chunked_mega_inline_vs_cpu_mirror() {
    for suffix_len in [65usize, 130, 200] {
        mega_inline_vs_cpu_mirror_impl(suffix_len);
    }
}

// Runs the Mode S prep-only precompute on Metal and reads back q_norm, k_norm,
// beta and the RAW per-token log_decay the Mode S kernel consumes directly
// (no cumsum / gram / solve dispatches). State-independent.
#[allow(clippy::type_complexity)]
fn run_mode_s_prep_metal(
    in_proj: &[bf16],
    a_log: &[f32],
    dt_bias: &[f32],
    num_v_heads: usize,
    ...
    (
        ...
        crate::tests::helpers::allocation_to_vec(&k_norm),
        crate::tests::helpers::allocation_to_vec(&beta),
        crate::tests::helpers::allocation_to_vec(&log_decay),
    )
}

// Test 2: the 3-dispatch Mode S pipeline (prep -> megaApply(inline)) vs the
// recurrent reference DeltaNetPrefill, at qwen3.5 shapes. Small-T focus with
// partial chunks; zero + nonzero initial state. SAME tolerances as Mode L.
fn mode_s_pipeline_vs_recurrent_impl(suffix_len: usize) {
    let num_v_heads = 48usize;
    let num_k_heads = 16usize;
    let head_k_dim = 128usize;
    let head_v_dim = 128usize;
    ...
    let (q_norm, k_norm, beta, log_decay) =
        run_mode_s_prep_metal(&in_proj, &a_log, &dt_bias, num_v_heads, num_k_heads, key_dim, value_dim, suffix_len);
    ...
    for nonzero_init in [false, true] {
        ...
        let (mega_out, mega_state) = run_mega_apply_inline::<Metal>(
            &q_norm,
            &k_norm,
            &in_proj,
            &log_decay,
        ...
        let (state_abs, state_rel) = max_abs_rel_err(&ref_state, &mega_state);
        eprintln!(
            "MODE_S_PIPELINE T={suffix_len} nonzero_init={nonzero_init}: \
             out max_abs={out_abs:.3e} max_rel={out_rel:.3e} \
             state max_abs={state_abs:.3e} max_rel={state_rel:.3e}"
        );

        let label = format!("mode S pipeline T={suffix_len} nonzero_init={nonzero_init}");
        assert_close(&ref_out, &mega_out, 2e-3, 5e-2, &format!("{label} out"));
        assert_close(&ref_state, &mega_state, 1e-3, 5e-2, &format!("{label} state"));
    }
}
...
#[uzu_test]
fn test_delta_net_chunked_mode_s_pipeline_vs_recurrent() {
    for suffix_len in [1usize, 63, 64, 65, 128, 200, 256] {
        mode_s_pipeline_vs_recurrent_impl(suffix_len);
    }
}
...
fn bench_delta_net_mode_l_vs_fused_prefill() {
    ...
    for suffix_len in [64usize, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768] {
        ...
        // Mode S only buffers (prep outputs consumed directly; no cumsum / gram /
        // solve / solveT). Mode S is VT=16 and small-T only (redundant precompute
        // across v-slices makes it uncompetitive past a few chunks).
        let mut q_norm_s = alloc(DataType::F32, suffix_len * key_dim);
        let mut k_norm_s = alloc(DataType::F32, suffix_len * key_dim);
        let mut beta_s = alloc(DataType::F32, suffix_len * num_v_heads);
        let mut log_decay_s = alloc(DataType::F32, suffix_len * num_v_heads);
        let mut mode_s_state = alloc(DataType::F32, state_size);
        let mut mode_s_out = alloc(DataType::BF16, suffix_len * value_dim);
        ...
        .unwrap();
        let mega_inline_k = <<Metal as Backend>::Kernels as Kernels>::DeltaNetChunkedMegaApplyInlineKernel::new(
            &context,
            DataType::BF16,
            DataType::BF16,
            16,
        ...
        // Mode S: prep -> megaApply(inline) -> normGate (3 dispatches).
        let mut run_mode_s = || {
            let mut encoder = Encoder::new(context.as_ref()).expect("encoder");
            encoder.encode_fill(&mut mode_s_state, 0);
            prep_k.encode(
                in_proj_array.allocation(),
                a_log_array.allocation(),
                dt_bias_array.allocation(),
        ...
        // Mode S (VT=16, inline precompute) is the small-T play only; its
        // redundant per-v-slice precompute makes it uncompetitive for many
        // chunks, so only measure it at small T.
        if suffix_len <= 512 {
            warmup(&mut run_mode_s);
            let mode_s_result = run_perf_with_warmup("mode_s", 3, iterations, &mut run_mode_s);
            print_row("mode_s", &mode_s_result);
            eprintln!(
                "MODESVERDICT\tT={}\tmode_s={:.3}\tvs_recurrent={:.3}x\tvs_mode_l={:.3}x\tvs_fused7={:.3}x",
                suffix_len,
                mode_s_result.median_ms,
                recurrent_result.median_ms / mode_s_result.median_ms,
                mode_l_result.median_ms / mode_s_result.median_ms,
                fused32_result.median_ms / mode_s_result.median_ms
            );
        }
    }
}
