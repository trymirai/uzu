...
impl<B: Backend> ClassifierLayer<B> {
    pub fn new(
        ...
        let (mlp, mlp_hadamard_factors, mlp_adapter_down_prime) = <dyn Mlp<B>>::new(
        ...
        assert!(mlp_adapter_down_prime.is_none(), "classifier layer does not support fused A_down RMSNorm");
