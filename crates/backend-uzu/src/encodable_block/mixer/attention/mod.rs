...
use crate::{
    ...
    encodable_block::{
        ...
        mixer::{
            Mixer, MixerState, MixerTokenTopology,
            attention::{
...
impl<B: Backend> Mixer<B> for Attention<B> {
    ...
    fn encode(
        ...
        let (hidden, gate) = if let Some(gate_projection) = &self.gate_projection {
            ...
            let gate = gate_projection.encode(hidden, batch_dim, encoder)?;
            (hidden_copy, Some(gate))
        } else {
            (hidden, None)
        };

        let mut qkv = self.qkv_projection.encode(hidden, batch_dim, encoder)?;
        ...
        let mut attention_output = if let Some(mut state) = state {
            ...
            let (core, trie) = match token_topology {
                MixerTokenTopology::Flat => (&self.flat_core, None),
                MixerTokenTopology::Trie {
                    token_subtrie_ranges,
                    ..
                } => (&self.trie_core, Some(*token_subtrie_ranges)),
            };
            ...
            let MixerTokenTopology::Flat = token_topology else {
                panic!("stateless attention doesn't support trie");
            };

            let mut keys =
                encoder.allocate_scratch(size_for_shape(&[batch_dim, num_kv_heads, self.head_dim], self.data_type))?;
            let mut values =
                encoder.allocate_scratch(size_for_shape(&[batch_dim, num_kv_heads, self.head_dim], self.data_type))?;
        ...
        if let Some(gate_kernel) = &self.gate_kernel {
            gate_kernel.encode(
                ...
                (batch_dim * self.num_q_heads * self.head_dim) as u32,
                encoder,
            );
        }
