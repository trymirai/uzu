...
impl<B: Backend> Mixer<B> for DeltaNet<B> {
    ...
    fn encode(
        ...
        if !matches!(token_topology, MixerTokenTopology::Flat) {
            panic!("delta net doesn't support trie");
        }
        ...
        let mut in_projected = self.in_projection.encode(hidden, batch_dim, encoder)?;

        let mut delta_output =
            encoder.allocate_scratch(size_for_shape(&[batch_dim, self.value_dim], self.outer_data_type))?;
        if batch_dim == 1 {
            ...
            let mut padded = encoder.allocate_scratch(size_for_shape(
                &[batch_dim + self.kernel_size - 1, self.total_proj_dim],
                DataType::F32,
            ))?;
            ...
            let mut prep_q_norm =
                encoder.allocate_scratch(size_for_shape(&[batch_dim, self.key_dim], DataType::F32))?;
            let mut prep_k_norm =
                encoder.allocate_scratch(size_for_shape(&[batch_dim, self.key_dim], DataType::F32))?;
            let mut prep_beta =
                encoder.allocate_scratch(size_for_shape(&[batch_dim, self.num_heads], DataType::F32))?;
            let mut prep_decay =
                encoder.allocate_scratch(size_for_shape(&[batch_dim, self.num_heads], DataType::F32))?;
        ...
        self.out_projection.encode(delta_output, batch_dim, encoder)
    }
}
