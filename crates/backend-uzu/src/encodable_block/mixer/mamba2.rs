...
impl<B: Backend> Mixer<B> for Mamba2<B> {
    ...
    fn encode(
        ...
        if !matches!(token_topology, MixerTokenTopology::Flat) {
            panic!("mamba2 doesn't support trie");
        }
        ...
        let mut conv_inputs =
            encoder.allocate_scratch(size_for_shape(&[batch_dim, self.conv_dim], self.inner_data_type))?;
        let mut gate = encoder
            .allocate_scratch(size_for_shape(&[batch_dim, self.num_heads, self.head_dim], self.inner_data_type))?;
        let mut time_step =
            encoder.allocate_scratch(size_for_shape(&[batch_dim, self.num_heads], self.inner_data_type))?;
        ...
        let mut state_b = encoder
            .allocate_scratch(size_for_shape(&[batch_dim, self.num_groups, self.state_dim], self.inner_data_type))?;
        ...
        if batch_dim == 1 {
            ...
            let mut padded = encoder
                .allocate_scratch(size_for_shape(&[batch_dim + state_stride, self.conv_dim], self.inner_data_type))?;
        ...
        let mut ssd_output =
            encoder.allocate_scratch(size_for_shape(&[batch_dim, self.inner_dim], self.inner_data_type))?;
        ...
        state.suffix_length = Some(batch_dim);

        self.out_projection.encode(ssd_output, batch_dim, encoder)
    }
}
