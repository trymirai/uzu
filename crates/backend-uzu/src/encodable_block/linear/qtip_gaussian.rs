...
use crate::{
    backends::common::{
        ...
        kernel::qtip_s_exact::{ComputedCodebook, QtipGaussianArguments, QtipSExactKernel},
    },
...
pub struct QtipGaussianLinear<B: Backend> {
    ...
    table_mode: u32,
    codebook_scale: f32,
    /// (scale, float4 offsets) when the codebook is generated by the state hash
    computed: Option<(f32, Allocation<B>)>,
    scales: Allocation<B>,
    gains: Allocation<B>,
    signs: Allocation<B>,
...
/// Level of each byte of fmix32(state * A + B), as `qtip_race_levels` in qtip_race.metal computes it.
fn hashed_levels(state: u32) -> [i32; 4] {
    ...
    std::array::from_fn(|byte| {
        let b = (x >> (8 * byte)) & 0xFF;
        let pairs = (0..4).map(|field| (b >> (2 * field)) & 3).sum::<u32>();
        (8 * pairs + ((3 * (b & 15)) & 15)) as i32 - 54
    })
}

/// (scale, offsets) when every entry of a 65536-state table is `scale * level + offsets[component]` for the hashed
/// levels, i.e. the table is a computed codebook the kernels can evaluate instead of reading.
fn computed_codebook(
    values: &[f32],
    vector_width: usize,
) -> Option<(f32, [f32; 4])> {
    if values.len() != 65_536 * vector_width {
        return None;
    }
    let levels: Vec<[i32; 4]> = (0..65_536u32).map(hashed_levels).collect();
    // least squares with one scale and one offset per component, in f64
    let level_means: Vec<f64> =
        (0..vector_width).map(|c| levels.iter().map(|level| level[c] as f64).sum::<f64>() / 65_536.0).collect();
    let value_means: Vec<f64> = (0..vector_width)
        .map(|c| (0..65_536).map(|state| values[state * vector_width + c] as f64).sum::<f64>() / 65_536.0)
        .collect();
    let (mut covariance, mut variance) = (0.0f64, 0.0f64);
    for state in 0..65_536 {
        for component in 0..vector_width {
            let level = levels[state][component] as f64 - level_means[component];
            covariance += level * (values[state * vector_width + component] as f64 - value_means[component]);
            variance += level * level;
        }
    }
    let scale = covariance / variance;
    let offsets: Vec<f64> = (0..vector_width).map(|c| value_means[c] - scale * level_means[c]).collect();
    let exact = (0..65_536).all(|state| {
        (0..vector_width).all(|c| {
            let model = scale * levels[state][c] as f64 + offsets[c];
            (values[state * vector_width + c] as f64 - model).abs() <= 1e-5
        })
    });
    exact.then(|| (scale as f32, std::array::from_fn(|class| offsets[class % vector_width] as f32)))
}
...
impl<B: Backend> QtipGaussianLinear<B> {
    pub fn load(
        ...
        let computed = if table_mode == 0 && state_count == 65_536 {
            match computed_codebook(&codebook_values[..], spec.vector_width as usize) {
                Some((scale, offsets)) => {
                    let mut allocation = context
                        .create_allocation(std::mem::size_of_val(&offsets), AllocationType::Global)
                        .map_err(QtipGaussianLinearError::BackendError)?;
                    allocation.copyin(&offsets);
                    Some((scale, allocation))
                },
                None => None,
            }
        } else {
    ...
    fn encode(
        ...
        let output = self.kernel.encode_qtip_gaussian(
            QtipGaussianArguments {
                ...
                computed: self.computed.as_ref().map(|(scale, offsets)| ComputedCodebook {
                    scale: *scale,
                    offsets,
                }),
                scales: &self.scales,
