...
use crate::{
    ...
    data_type::DataType,
    encodable_block::{
        mixer::{MixerState, MixerTokenTopology, attention::rope::PrecalculatedRoPE},
...
impl<B: Backend> TransformerState<B> {
    ...
    pub fn max_context_length(&self) -> Option<usize> {
        self.layer_states
            .iter()
            .filter_map(|layer_state| {
                if let TransformerLayerStateType::Owned(layer_state) = layer_state {
                    Some(layer_state.max_context_length())
                } else {
                    None
                }
            })
            .fold(None, |acc, el| match (acc, el) {
...
impl<B: Backend> Transformer<B> {
    pub fn new(
        ...
        let layers = transformer_config
            ...
            .map(|(layer_index, layer_config)| {
                ...
                let rope = if let Some(layer_rope_config) = &layer_config.rope_config {
                    Some(ropes.iter().position(|rope_config| rope_config == layer_rope_config).unwrap_or_else(|| {
    ...
    pub fn trie_supported(&self) -> bool {
        self.layers.iter().all(|(layer, _rope)| layer.mixer.trie_supported())
    }
    ...
    pub fn encode(
        ...
        let token_positions: Vec<usize> = match token_topology {
            MixerTokenTopology::Flat => (context_length..(context_length + batch_dim)).into_iter().collect(),
            MixerTokenTopology::Trie {
                token_positions,
                ..
            } => token_positions.iter().map(|rel_pos| context_length + *rel_pos).collect(),
        };
