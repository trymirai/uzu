    ...
    pub fn new(
        ...
        let decoding_state = if !input.is_empty() {
            ...
            let (before_snapshot, after_snapshot) =
                input.split_at(options.snapshot_position.map_or(0, |position| position - prefill_range.start));
            ...
            for (input_chunk, sample_last) in batches
                ...
                .flatten()
            {
                if options.snapshot_position == Some(model_state.tokens.len()) {
                    ...
                    model_state.snapshot_position = options.snapshot_position;
                }
