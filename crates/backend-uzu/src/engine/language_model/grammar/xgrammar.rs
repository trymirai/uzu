...
impl Into<GrammarError> for XGrammarError {
...
impl XGrammar {
    pub fn new(
        ...
        let tokenizer_info = TokenizerInfo::from_huggingface(tokenizer, None, stop_token_ids)
            .map_err(|e| XGrammarError::XGrammar(e).into())?;
        ...
        let grammar = match config {
            ...
            } => XGrammarGrammar::from_regex(pattern, *print_converted_ebnf)
                .map_err(|e| XGrammarError::XGrammar(e).into())?,
            GrammarConfig::BuiltinJson => XGrammarGrammar::builtin_json_grammar(),
        };
        let mut compiler =
            GrammarCompiler::new(&tokenizer_info, 8, true, -1).map_err(|e| XGrammarError::XGrammar(e).into())?;
        let compiled = compiler.compile_grammar(&grammar).map_err(|e| XGrammarError::XGrammar(e).into())?;
        let matcher = GrammarMatcher::new(&compiled, None, true, -1).map_err(|e| XGrammarError::XGrammar(e).into())?;
