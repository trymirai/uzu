...
use xgrammar::{
    DLDataType, DLDataTypeCode, DLDevice, DLDeviceType, DLTensor, Grammar as XGrammarGrammar, GrammarCompiler,
    GrammarMatcher, TokenizerInfo, c_void,
};

use crate::{
    data_type::DataType,
    engine::language_model::grammar::{Grammar, GrammarConfig, GrammarError},
};

enum GrammarEngagementState {
    Always,
    Triggered {
...
impl From<XGrammarError> for GrammarError {
    fn from(val: XGrammarError) -> Self {
        GrammarError(Box::new(val))
    }
}
...
impl XGrammar {
    pub fn new(
        ...
        let tokenizer_info =
            TokenizerInfo::from_huggingface(tokenizer, None, stop_token_ids).map_err(XGrammarError::XGrammar)?;
        ...
        let grammar = match config {
            ...
            } => XGrammarGrammar::from_regex(pattern, *print_converted_ebnf).map_err(XGrammarError::XGrammar)?,
            GrammarConfig::BuiltinJson => XGrammarGrammar::builtin_json_grammar(),
        };
        let mut compiler = GrammarCompiler::new(&tokenizer_info, 8, true, -1).map_err(XGrammarError::XGrammar)?;
        let compiled = compiler.compile_grammar(&grammar).map_err(XGrammarError::XGrammar)?;
        let matcher = GrammarMatcher::new(&compiled, None, true, -1).map_err(XGrammarError::XGrammar)?;
...
impl Grammar for XGrammar {
    ...
    fn accept_token(
        ...
        if self.engagement_state.is_engaged() && !self.matcher.accept_token(token_id as i32) {
            return Err(XGrammarError::GrammarReject.into());
        }
