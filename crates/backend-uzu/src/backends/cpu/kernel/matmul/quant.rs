    ...
    backends::{
        common::{
            ...
            kernel::quant_matmul::{
                ...
                QuantizedMatmulError,
            },
        },
        cpu::{Cpu, context::CpuContext, kernel::quant_matmul::qmm_transposed::qmm_transposed},
    },
    utils::pointers::{SendPtr, SendPtrMut},
};

pub struct QuantizedGemmCpuKernel {
    configuration: QuantizedMatmulConfiguration,
}

impl QuantizedGemmKernel for QuantizedGemmCpuKernel {
    type Backend = Cpu;

    fn new(
        _context: &CpuContext,
        configuration: QuantizedMatmulConfiguration,
    ) -> Result<Self, QuantizedMatmulError<Cpu>> {
        if !matches!(
    ...
    fn encode(
        ...
        let bits: usize = match self.configuration.mode {
            QuantizationMode::U4 => 4,
            QuantizationMode::I8 | QuantizationMode::U8 => 8,
        };
        let quant_method = self.configuration.quantization_method;
        let group_size = self.configuration.group_size;
        let in_vec_size = self.configuration.input_dim;
        let out_vec_size = self.configuration.output_dim;
        let batch_size = batch_dim;
        let data_type = self.configuration.data_type;
        ...
        let zp_ptr =
            SendPtr(unsafe { &*zp_buf.buffer().get() }.as_ptr().wrapping_byte_add(zp_byte_off));
        ...
        command_buffer.push_command(move || match data_type {
            DataType::F32 => run::<f32>(
                a_ptr, b_ptr, scales_ptr, zp_ptr, d_ptr, in_vec_size, out_vec_size,
                batch_size, quant_method, group_size, bits,
            ),
...
fn run<T: ArrayElement + Float>(
    ...
    out_vec_size: usize,
    batch_size: usize,
    quant_method: crate::backends::common::gpu_types::QuantizationMethod,
    group_size: usize,
    bits: usize,
) {
