...
pub fn weaver_top_children(
    ...
    output_dflash_logprobs: *mut f32,
    rows: u32,
    candidates: u32,
    expand_width: u32,
) {
    ...
    for row in 0..rows {
        ...
        let dflash_logits =
            (0..candidates).map(|index| unsafe { *candidate_logits.add(base + index) }).collect::<Vec<_>>();
        ...
        let dflash_max = dflash_logits.iter().copied().fold(f32::NEG_INFINITY, f32::max);
        let dflash_log_sum =
            dflash_logits.iter().map(|value| (value - dflash_max).exp()).sum::<f32>().ln() + dflash_max;
        let mut indices = (0..candidates).collect::<Vec<_>>();
        indices
            .sort_by(|&left, &right| logits[right].total_cmp(&logits[left]).then_with(|| token(left).cmp(&token(right))));
        for (rank, index) in indices.into_iter().take(expand_width).enumerate() {
            unsafe {
                ...
                *output_dflash_logprobs.add(row * expand_width + rank) = dflash_logits[index] - dflash_log_sum;
            }
        }
    }
}
