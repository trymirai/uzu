...
pub fn build_prefix_beta<T: ArrayElement + Float>(
    ...
    unsafe {
        for batch in 0..batch_size {
            ...
            let tensor_batch = batch * tree_size * value_heads;

            for row in 0..tree_size {
                for head in 0..value_heads {
                    let out_idx = tensor_batch + row * value_heads + head;
                    ...
                    *beta.add(out_idx) = 1.0 / (1.0 + (-b_val).exp());
                    ...
                    for col in 0..tree_size {
                        ...
                        let a_val = (*a.add(tensor_batch + col * value_heads + head)).to_f32().unwrap();
