...
pub fn weaver_top_children(
    ...
    let children = (children as usize).min(candidates);
    for row in 0..rows {
        let base = row * candidates;
        let logits = (0..candidates)
            .map(|index| unsafe { *candidate_scores.add(base + index) + *residual_logits.add(base + index) })
            .collect::<Vec<_>>();
        ...
        for (rank, index) in indices.into_iter().take(children).enumerate() {
            unsafe {
                *output_ids.add(row * children + rank) = *candidate_ids.add(base + index) as u32;
