...
impl WebGPUCommandBufferEncoding {
    ...
    pub(super) fn ensure_compute<'a>(&'a mut self) -> &'a mut wgpu::ComputePass<'static> {
...
impl CommandBufferPending for WebGPUCommandBufferPending {
    ...
    fn wait_until_completed(self) -> Result<WebGPUCommandBufferCompleted, WebGPUError> {
        while let Err(try_recv_err) = self.completed_signal.try_recv() {
            match try_recv_err {
                TryRecvError::Empty => self.context.device.poll(wgpu::PollType::Poll).ok(),
                TryRecvError::Disconnected => return Err(WebGPUError::CommandBufferFailed),
            };
        }
...
impl CommandBufferCompleted for WebGPUCommandBufferCompleted {
    ...
    fn gpu_execution_time(&self) -> Duration {
        Duration::ZERO // TODO: refactor required
    }
}
