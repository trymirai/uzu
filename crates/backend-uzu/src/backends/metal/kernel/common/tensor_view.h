...
struct TensorView3D {
  ...
  TensorView3D(device T* ptr, int d0, int d1, int d2) : data(ptr), dim0(d0), dim1(d1), dim2(d2) {
    stride0 = d1 * d2;
    stride1 = d2;
    stride2 = 1;
  }
  ...
  device T& at(uint i, uint j, uint k) const { return data[i * stride0 + j * stride1 + k * stride2]; }
