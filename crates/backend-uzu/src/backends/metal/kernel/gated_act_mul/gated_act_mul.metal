...
PUBLIC KERNEL(GatedActMul) (
    ...
    const constant float& act_clip_min OPTIONAL(clip_act),
    const constant float& act_clip_max OPTIONAL(clip_act),
    ...
    const bool clip_act SPECIALIZE,
  ...
  if (clip_act) {
    gate = static_cast<T>(clamp(float(gate), act_clip_min, act_clip_max));
  }
  if (clip_value) {
