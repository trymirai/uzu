...
PUBLIC KERNEL(AttentionUpdateKVCache)(
  ...
  const uint cacheOffset = keys_in_place
                               ? ((group_index * max_sequence_length + cacheTokenIndex) * head_dim + dim_index)
                               : ((cacheTokenIndex * num_groups + group_index) * head_dim + dim_index);

  const uint rotatedKeyOffset = (group_index * suffix_length + token_index) * head_dim + dim_index;
  ...
  const uint valueOffset = token_index * qkvStride + valueBaseOffset + group_index * head_dim + dim_index;
