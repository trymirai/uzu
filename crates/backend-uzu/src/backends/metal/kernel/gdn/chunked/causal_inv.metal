...
KERNEL(DeltaNetChunkedCausalInv)(
  ...
  const device float* inv_blocks =
      a_inv + (chunk_idx * num_v_heads + hv_idx) * num_blocks * (DIAG_BLOCK_SIZE * DIAG_BLOCK_SIZE);
  ...
  for (uint block_idx = 0; block_idx < num_blocks; ++block_idx) {
    ...
    // a_packed stays positive for block inverse; dense solve subtracts it.
    ...
    for (uint pair_idx = 0; pair_idx < num_full_pairs; ++pair_idx) {
      ...
      const device float* a_pair =
          a_blocks + (block_idx * num_col_pairs + pair_idx) * (DIAG_BLOCK_SIZE * 2 * DIAG_BLOCK_SIZE);
      a_frag.load_from(lane, fragment_source(a_pair, 2 * DIAG_BLOCK_SIZE));
      a_frag.map([](float value) { return -value; });

      PairValueFragment t_prev_frag;
      const device bfloat* t_prev = t_head + pair_idx * 2 * DIAG_BLOCK_SIZE * CHUNK_SIZE;
      t_prev_frag.load_from(lane, fragment_source(t_prev, CHUNK_SIZE));
    ...
    for (uint prev_block_idx = num_full_pairs * 2; prev_block_idx < block_idx; ++prev_block_idx) {
      ...
      const device float* a_block =
          a_blocks + (block_idx * num_col_pairs + prev_block_idx / 2) * (DIAG_BLOCK_SIZE * 2 * DIAG_BLOCK_SIZE) +
          (prev_block_idx % 2) * DIAG_BLOCK_SIZE;
      a_frag.load_from(lane, fragment_source(a_block, 2 * DIAG_BLOCK_SIZE));
      a_frag.map([](float value) { return -value; });

      ValueFragment t_prev_frag;
      const device bfloat* t_prev = t_head + prev_block_idx * DIAG_BLOCK_SIZE * CHUNK_SIZE;
      t_prev_frag.load_from(lane, fragment_source(t_prev, CHUNK_SIZE));
      fragment_mma(acc_t, a_frag, t_prev_frag);
    }
    ...
    inv_frag.load_from(
        lane,
        fragment_source(inv_blocks + block_idx * DIAG_BLOCK_SIZE * DIAG_BLOCK_SIZE, DIAG_BLOCK_SIZE)
    );

    TileFragment t_block;
    t_block.clear();
    fragment_mma(t_block, inv_frag, acc_t);
    t_block.store(lane, t_head + row_base * CHUNK_SIZE, CHUNK_SIZE);
