...
KERNEL(DeltaNetChunkedPackedAAndDiaInv)(
    ...
    const uint block_idx GROUPS(CHUNK_SIZE.div_ceil(DIAG_BLOCK_SIZE)),
  ...
  for (uint idx = lane; idx < num_col_pairs * DIAG_BLOCK_SIZE * 2 * DIAG_BLOCK_SIZE; idx += METAL_SIMD_SIZE) {
    const uint local_col = idx % (2 * DIAG_BLOCK_SIZE);
    const uint local_row = (idx / (2 * DIAG_BLOCK_SIZE)) % DIAG_BLOCK_SIZE;
    const uint pair_idx = idx / (DIAG_BLOCK_SIZE * 2 * DIAG_BLOCK_SIZE);
    const uint row = row_base + local_row;
    const uint col = pair_idx * 2 * DIAG_BLOCK_SIZE + local_col;
    ...
    if (pair_idx == diag_pair && local_col >= diag_col_offset && local_col < diag_col_offset + DIAG_BLOCK_SIZE) {
