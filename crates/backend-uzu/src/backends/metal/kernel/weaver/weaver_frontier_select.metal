...
PUBLIC KERNEL(WeaverFrontierSelect)(
    ...
    threadgroup uint row_candidate_pool_row[WEAVER_FRONTIER_MAX_WIDTH],
  ...
  if (lid < width) {
    ...
    const uint cumulative_logprob = real ? frontier[WEAVER_FRONTIER_LANE_CUM * capacity + slot] : 0u;
    ...
    tree[WEAVER_TREE_LANE_CUM * tree_slots + tree_slot] = cumulative_logprob;
  ...
  for (uint row = 0; row < width; ++row) {
    ...
    for (uint candidate = lid; candidate < candidate_pool_size; candidate += WEAVER_FRONTIER_SELECT_THREADS) {
