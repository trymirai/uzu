...
// NF4 QMV using `constant` address-space codebook, with per-group scales
// stored as 1-byte E4M3 FP8 (half the bytes of the bf16-scale variant).
// Structure mirrors Nf4QmvConstant exactly; only the `scales` buffer type
// (uint8_t instead of T) and the per-group scale decode differ.
...
KERNEL(Nf4QmvE4m3)(
  ...
  for (uint k = 0; k < in_vec_size; k += block_size) {
    ...
    uint8_t s0 = scales[0];
    uint8_t s1 = scales[in_vec_size_g];
    uint8_t s2 = scales[2 * in_vec_size_g];
    uint8_t s3 = scales[3 * in_vec_size_g];

    result[0] += qdot_nf4_e4m3<values_per_thread>(wl0, x_thread, s0);
    result[1] += qdot_nf4_e4m3<values_per_thread>(wl1, x_thread, s1);
    result[2] += qdot_nf4_e4m3<values_per_thread>(wl2, x_thread, s2);
    result[3] += qdot_nf4_e4m3<values_per_thread>(wl3, x_thread, s3);
