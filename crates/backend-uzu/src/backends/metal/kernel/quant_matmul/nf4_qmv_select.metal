...
// NF4 QMV using a ZERO-MEMORY in-thread *select* codebook.
//
// Every thread "holds" all 16 NF4 codebook values as compile-time constants;
// per-weight dequant is a switch-of-literals on the 4-bit nibble
// (`nf4_select_entry`) — NOT a register array indexed dynamically (which
// would spill), NO `constant`/`threadgroup` codebook, NO cross-lane op.
// Numerically identical to `Nf4QmvConstant` (same 16 half literals; only the
// value *source* differs: in-register select vs constant-space gather).
// Same VARIANTS/signature/scale handling as `Nf4QmvConstant`
// (T=bfloat, GROUP_SIZE=64, bf16 per-group scale, no zero_points/bias).
...
KERNEL(Nf4QmvSelect)(
  ...
  for (uint k = 0; k < in_vec_size; k += block_size) {
    ...
    result[0] += qdot_nf4_select<values_per_thread>(wl0, x_thread, s0);
    result[1] += qdot_nf4_select<values_per_thread>(wl1, x_thread, s1);
    result[2] += qdot_nf4_select<values_per_thread>(wl2, x_thread, s2);
    result[3] += qdot_nf4_select<values_per_thread>(wl3, x_thread, s3);
