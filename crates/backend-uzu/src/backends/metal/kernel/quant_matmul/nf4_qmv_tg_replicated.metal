...
// NF4 QMV using a 4x replicated threadgroup codebook to spread access across
// all 32 tgmem banks. Layout: 64 half entries = 4 copies x 16 entries = 128 B.
// Each copy spans 8 banks (entries 0..1 share a bank); the 4 copies occupy
// disjoint bank ranges {0..7, 8..15, 16..23, 24..31}.
//
// Each lane reads from copy `simd_lane >> 3`, so lanes 0..7 hit banks 0..7,
// lanes 8..15 hit banks 8..15, etc. This eliminates the "8 idle banks"
// problem of plain Nf4QmvTg (LUT[16] = 32 B, only banks 0..7 active).
//
// Inner loop is byte-identical to qdot_nf4_tg — only the per-lane base
// pointer into the replicated table differs.
template <typename T, uint GROUP_SIZE>
VARIANTS(T, bfloat)
VARIANTS(GROUP_SIZE, 64)
KERNEL(Nf4QmvTgReplicated)(
    ...
    threadgroup half codebook_tg_rep[64],
  ...
  // Cooperative init: 64 entries, 4 copies of the 16-value NF4 codebook.
  ...
  for (uint i = tid; i < 64u; i += tgp_size) {
    codebook_tg_rep[i] = nf4_codebook[i & 0x0fu];
  }
  threadgroup_barrier(mem_flags::mem_threadgroup);

  // Per-lane base pointer into one of 4 copies (8 lanes share a copy).
  const threadgroup half* my_cb = codebook_tg_rep + ((simd_lane >> 3) * 16u);
