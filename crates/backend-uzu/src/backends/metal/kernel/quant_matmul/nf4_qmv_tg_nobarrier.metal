...
// =============================================================================
// PERF-ONLY PROBE KERNEL — OUTPUTS ARE INTENTIONALLY INCORRECT.
//
// This is a byte-for-byte copy of `Nf4QmvTg`, with the
// `threadgroup_barrier(mem_flags::mem_threadgroup)` after the cooperative
// codebook init REMOVED. The barrier removal is the ONLY change; cooperative
// init is preserved so this isolates barrier cost specifically (vs init cost).
//
// Because the barrier is the only thing that orders the codebook stores with
// the subsequent codebook loads in `qdot_nf4_tg`, this kernel is racy and its
// output values are garbage. DO NOT use it for correctness. DO NOT route any
// real model through it. The ONLY purpose is to bound the kernel-entry cost
// of the threadgroup barrier on M4 so we can decide whether to invest in a
// simdgroup-local LUT variant.
// =============================================================================
template <typename T, uint GROUP_SIZE>
VARIANTS(T, bfloat)
VARIANTS(GROUP_SIZE, 64)
KERNEL(Nf4QmvTgNoBarrier)(
  ...
  // Cooperative init of the threadgroup codebook (intentionally NO barrier
  // — see file header).
  ...
  // INTENTIONALLY OMITTED: threadgroup_barrier(mem_flags::mem_threadgroup);
  ...
  for (uint k = 0; k < in_vec_size; k += block_size) {
    ...
    result[0] += qdot_nf4_tg<values_per_thread>(wl0, x_thread, codebook_tg, s0);
    result[1] += qdot_nf4_tg<values_per_thread>(wl1, x_thread, codebook_tg, s1);
    result[2] += qdot_nf4_tg<values_per_thread>(wl2, x_thread, codebook_tg, s2);
    result[3] += qdot_nf4_tg<values_per_thread>(wl3, x_thread, codebook_tg, s3);
