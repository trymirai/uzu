...
enum Nf4Mode {
  NF4_CONST = 1,
  NF4_TG = 3,
  NF4_ZP = 4, // constant codebook + per-group zero-point LUT offset
};

template <Nf4Mode MODE>
struct Nf4LookupCtx {
  threadgroup const half* tg_cb; // valid for TG; unused for CONST/ZP
};
...
// NF4-ZP dequantize: like nf4_dequantize but adds a per-group zero-point
// offset `zp_off` to each codebook value before the scale multiply.
template <typename U, int N>
inline void nf4_dequantize_zp(
    const device uint8_t* w,
    U scale,
    U zp_off,
    threadgroup U* w_local
  ...
  for (int i = 0; i < (N / 2); i++) {
    uint8_t byte = w[i];
    U h0 = static_cast<U>(nf4_codebook[byte & 0x0f]) + zp_off;
    U h1 = static_cast<U>(nf4_codebook[(byte >> 4) & 0x0f]) + zp_off;
    w_local[2 * i] = h0 * scale;
    w_local[2 * i + 1] = h1 * scale;
  }
}
...
// NF4-ZP block loader: forks Nf4BlockLoader, adding a per-group zero-point
// offset (4-bit index into nf4_zp_lut, packed two-per-byte row-major) added
// to each codebook value before the scale multiply.
template <
    typename T,
    short BROWS,
    short BCOLS,
...
struct Nf4ZpBlockLoader {
  ...
  const device uint8_t* src;
  const device T* scales;
  // zp row base for this loader's row (already offset to the absolute output
  // neuron), plus a running K-group index kept in sync with `scales`.
  const device uint8_t* zp_row;
  ...
  Nf4ZpBlockLoader(
      ...
      : src_ld(src_ld_),
        ...
        zp_row(zero_points_ + uint(bi) * zp_stride_), zp_stride(zp_stride_),
        k_group(0u) {}

  T zp_off() const {
    uint8_t byte = zp_row[k_group >> 1];
    uint8_t idx =
        ((k_group & 1u) == 0u) ? (byte & 0x0fu) : ((byte >> 4) & 0x0fu);
    return static_cast<T>(nf4_zp_lut[idx]);
  }
  ...
  void load_unsafe() const {
    ...
    T zo = zp_off();
    for (int i = 0; i < n_reads; i++) {
      nf4_dequantize_zp<T, pack_factor>(
          src + i * bytes_per_pack,
          scale,
          zo,
          dst + i * pack_factor
...
inline void nf4_qmm_zp_impl(
  ...
  using loader_w_t =
      Nf4ZpBlockLoader<T, BN, BK, BK_padded, 1, WM * WN * 32, GROUP_SIZE>;

  const int in_vec_size_w = in_vec_size * bytes_per_pack / pack_factor;
  const int in_vec_size_g = (in_vec_size + GROUP_SIZE - 1) / int(GROUP_SIZE);
  const uint zp_stride = (uint(in_vec_size_g) + 1u) / 2u;
  ...
  const device uint8_t* zp_block = zero_points + uint(out_col) * zp_stride;
