#include <metal_stdlib>
#include "../common/dsl.h"
#include "nf4_qmm_core.h"

// NF4 transposed QMM with threadgroup-codebook lookup. BM/BK/BN/WM/WN are
// VARIANTS so the dispatcher can pick a small (BM=8) or large (BM=64) tile,
// matching the AWQ qmm_transposed sizing rules.
template <typename T, uint GROUP_SIZE, uint BM, uint BK, uint BN, uint WM, uint WN>
VARIANTS(T, bfloat)
VARIANTS(GROUP_SIZE, 64)
VARIANTS(BM, 8, 32, 64)
VARIANTS(BK, 32, 64)
VARIANTS(BN, 32, 64)
VARIANTS(WM, 1, 2)
VARIANTS(WN, 1, 2)
CONSTRAINT(
  (BM == 8  && BK == 32 && BN == 32 && WM == 1 && WN == 1) ||
  (BM == 32 && BK == 32 && BN == 32 && WM == 2 && WN == 2) ||
  (BM == 64 && BK == 64 && BN == 64 && WM == 2 && WN == 2))
CONSTRAINT(BK <= GROUP_SIZE)
KERNEL(Nf4QmmTg)(
    const device uint32_t* weights,
    const device T* scales,
    const device T* input,
    device T* output,
    const constant uint& in_vec_size,
    const constant uint& out_vec_size,
    const constant uint& batch_size,
    threadgroup T Xs[BM * (BK + 16 / sizeof(T))],
    threadgroup T Ws[BN * (BK + 16 / sizeof(T))],
    threadgroup half codebook_tg[16],
    const uint out_block_idx GROUPS(out_vec_size.div_ceil(BN)),
    const uint batch_block_idx GROUPS(batch_size.div_ceil(BM)),
    const uint simd_lane THREADS(32),
    const uint simd_group THREADS(WM * WN)
) {
  // Cooperatively initialize threadgroup codebook. Total threads = WM*WN*32.
  const uint tid = simd_group * 32u + simd_lane;
  const uint tgp_size = WM * WN * 32u;
  nf4_init_tg_codebook(codebook_tg, tid, tgp_size);
  threadgroup_barrier(mem_flags::mem_threadgroup);

  Nf4LookupCtx<NF4_TG> ctx;
  ctx.tg_cb = codebook_tg;

  nf4_qmm_impl<T, GROUP_SIZE, NF4_TG, true, BM, BK, BN, WM, WN>(
      weights,
      scales,
      input,
      output,
      Xs,
      Ws,
      ctx,
      in_vec_size,
      out_vec_size,
      batch_size,
      out_block_idx,
      batch_block_idx,
      simd_group,
      simd_lane
  );
}
