...
// REPLICATED `DUP` times in threadgroup memory (FLUTE's trick). Each lane
// picks its replica via `simd_lane & (DUP - 1)`, scattering accesses across
// banks to relieve L1/threadgroup-port saturation.
//
// Layout: pair_lut_dup[code * DUP + dup] holds the same half2 codebook pair
// for every dup ∈ [0, DUP). Only the address (and therefore bank) differs.
//
// DUP ∈ {8, 16, 32}. Threadgroup mem: 256 * DUP * 4 B (= 8/16/32 KiB).
// Same VARIANTS/signature shape as Nf4QmvByte256 (T=bfloat, GROUP_SIZE=64).
template <typename T, uint GROUP_SIZE, uint DUP>
VARIANTS(T, bfloat)
VARIANTS(GROUP_SIZE, 64)
VARIANTS(DUP, 8, 16, 32)
KERNEL(Nf4QmvByte256Dup)(
    ...
    threadgroup half2 pair_lut_dup[256 * DUP],
  ...
  // Cooperatively build the byte-batched 256-entry codebook LUT, replicated
  // DUP times. Each duplicate slot holds the same half2 value, only the
  // (bank) address differs.
  const uint tid = simd_group * METAL_SIMD_SIZE + simd_lane;
  const uint tgp_size = num_simdgroups * METAL_SIMD_SIZE;
  const uint total_entries = 256u * DUP;
  for (uint i = tid; i < total_entries; i += tgp_size) {
    const uint code = i / DUP;
    pair_lut_dup[i] =
        half2(nf4_codebook[code & 0x0fu], nf4_codebook[(code >> 4) & 0x0fu]);
  }
  threadgroup_barrier(mem_flags::mem_threadgroup);

  // Per-lane base offset into one of DUP copies. Assumes DUP is a power of 2.
  const uint dup = simd_lane & (DUP - 1u);
  ...
  for (uint k = 0; k < in_vec_size; k += block_size) {
    ...
    result[0] += qdot_nf4_byte_lut_dup<values_per_thread, DUP>(
        wl0,
        x_thread,
        pair_lut_dup,
        dup,
    ...
    result[1] += qdot_nf4_byte_lut_dup<values_per_thread, DUP>(
        wl1,
        x_thread,
        pair_lut_dup,
        dup,
    ...
    result[2] += qdot_nf4_byte_lut_dup<values_per_thread, DUP>(
        wl2,
        x_thread,
        pair_lut_dup,
        dup,
    ...
    result[3] += qdot_nf4_byte_lut_dup<values_per_thread, DUP>(
        wl3,
        x_thread,
        pair_lut_dup,
        dup,
