...
// NF4 QMV using `constant` address-space codebook.
template <typename T, uint GROUP_SIZE>
VARIANTS(T, bfloat)
VARIANTS(GROUP_SIZE, 64)
KERNEL(Nf4QmvConstant)(
  ...
  for (uint k = 0; k < in_vec_size; k += block_size) {
    ...
    result[0] += qdot_nf4_constant<values_per_thread>(wl0, x_thread, s0);
    result[1] += qdot_nf4_constant<values_per_thread>(wl1, x_thread, s1);
    result[2] += qdot_nf4_constant<values_per_thread>(wl2, x_thread, s2);
    result[3] += qdot_nf4_constant<values_per_thread>(wl3, x_thread, s3);
