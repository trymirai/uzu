...
void causal_conv_transpose1d_causal_pad(
  ...
  if ((input_layout != AUDIO_LAYOUT_NCS && input_layout != AUDIO_LAYOUT_NSC) || groups <= 0 || stride <= 0 ||
      kernel_size <= 0 || (cin % groups) != 0 || (cout % groups) != 0) {
    return;
  }
  ...
  const int seq_len_expanded = (seq_len_in > 0) ? (((seq_len_in - 1) * stride) + 1) : 0;
  ...
  if (valid_count > 0) {
    ...
    if (input_layout == AUDIO_LAYOUT_NCS) {
      for (int ic = ic_begin; ic < ic_end; ++ic) {
        ...
        const uint w_base = ((uint)oc * (uint)cin_per_group + ic_local) * (uint)kernel_size;
        if (fast_two_tap) {
          ...
          if (stride >= (int)AUDIO_TIME_TILE) {
            ...
            const float in_qm1 = (q0 > 0 && q0 - 1 < seq_len_in) ? float(input[in_base + (uint)(q0 - 1)]) : 0.0f;
            const float in_q0 = (q0 >= 0 && q0 < seq_len_in) ? float(input[in_base + (uint)q0]) : 0.0f;
            // Lane 0: r = r0
            if (valid_count > 0) {
              acc0 += in_qm1 * float(weight[w_base + (uint)((stride - 1) - r0)]);
              acc0 += in_q0 * float(weight[w_base + (uint)((kernel_size - 1) - r0)]);
            }
            // Lane 1: r = r0+1
            if (valid_count > 1) {
              const int r1 = r0 + 1;
              acc1 += in_qm1 * float(weight[w_base + (uint)((stride - 1) - r1)]);
              acc1 += in_q0 * float(weight[w_base + (uint)((kernel_size - 1) - r1)]);
            }
            // Lane 2: r = r0+2
            if (valid_count > 2) {
              const int r2 = r0 + 2;
              acc2 += in_qm1 * float(weight[w_base + (uint)((stride - 1) - r2)]);
              acc2 += in_q0 * float(weight[w_base + (uint)((kernel_size - 1) - r2)]);
            }
            // Lane 3: r = r0+3
            if (valid_count > 3) {
              const int r3 = r0 + 3;
              acc3 += in_qm1 * float(weight[w_base + (uint)((stride - 1) - r3)]);
              acc3 += in_q0 * float(weight[w_base + (uint)((kernel_size - 1) - r3)]);
            }
          } else {
            ...
            if (valid_count > 0) {
              ...
              if (q0 > 0 && q0 - 1 < seq_len_in) {
                acc0 += float(input[in_base + (uint)(q0 - 1)]) * float(weight[w_base + (uint)k_lo]);
              }
              if (q0 >= 0 && q0 < seq_len_in) {
                acc0 += float(input[in_base + (uint)q0]) * float(weight[w_base + (uint)k_hi]);
              }
            }
            if (valid_count > 1) {
              ...
              if (q1 > 0 && q1 - 1 < seq_len_in) {
                acc1 += float(input[in_base + (uint)(q1 - 1)]) * float(weight[w_base + (uint)((stride - 1) - r1)]);
              }
              if (q1 >= 0 && q1 < seq_len_in) {
                acc1 += float(input[in_base + (uint)q1]) * float(weight[w_base + (uint)((kernel_size - 1) - r1)]);
              }
            }
            if (valid_count > 2) {
              ...
              if (q2 > 0 && q2 - 1 < seq_len_in) {
                acc2 += float(input[in_base + (uint)(q2 - 1)]) * float(weight[w_base + (uint)((stride - 1) - r2)]);
              }
              if (q2 >= 0 && q2 < seq_len_in) {
                acc2 += float(input[in_base + (uint)q2]) * float(weight[w_base + (uint)((kernel_size - 1) - r2)]);
              }
            }
            if (valid_count > 3) {
              ...
              if (q3 > 0 && q3 - 1 < seq_len_in) {
                acc3 += float(input[in_base + (uint)(q3 - 1)]) * float(weight[w_base + (uint)((stride - 1) - r3)]);
              }
              if (q3 >= 0 && q3 < seq_len_in) {
                acc3 += float(input[in_base + (uint)q3]) * float(weight[w_base + (uint)((kernel_size - 1) - r3)]);
              }
            }
          }
        } else {
          for (int k = 0; k < kernel_size; ++k) {
            ...
            if (valid_count > 0) {
              ...
              if (expanded_time0 >= 0 && expanded_time0 < seq_len_expanded && (expanded_time0 % stride) == 0) {
            ...
            if (valid_count > 1) {
              ...
              if (expanded_time1 >= 0 && expanded_time1 < seq_len_expanded && (expanded_time1 % stride) == 0) {
            ...
            if (valid_count > 2) {
              ...
              if (expanded_time2 >= 0 && expanded_time2 < seq_len_expanded && (expanded_time2 % stride) == 0) {
            ...
            if (valid_count > 3) {
              ...
              if (expanded_time3 >= 0 && expanded_time3 < seq_len_expanded && (expanded_time3 % stride) == 0) {
        ...
        if (fast_two_tap) {
          if (stride >= (int)AUDIO_TIME_TILE) {
            ...
            const float in_qm1 = (q0 > 0 && q0 - 1 < seq_len_in)
                                     ? float(input[(b * (uint)seq_len_in + (uint)(q0 - 1)) * (uint)cin + (uint)ic])
                                     : 0.0f;
            const float in_q0 = (q0 >= 0 && q0 < seq_len_in)
                                    ? float(input[(b * (uint)seq_len_in + (uint)q0) * (uint)cin + (uint)ic])
                                    : 0.0f;
            if (valid_count > 0) {
              acc0 += in_qm1 * float(weight[w_base + (uint)((stride - 1) - r0)]);
              acc0 += in_q0 * float(weight[w_base + (uint)((kernel_size - 1) - r0)]);
            ...
            if (valid_count > 0) {
              if (q0 > 0 && q0 - 1 < seq_len_in) {
                ...
                acc0 += float(input[in_idx]) * float(weight[w_base + (uint)((stride - 1) - r0)]);
              }
              if (q0 >= 0 && q0 < seq_len_in) {
                const uint in_idx = (b * (uint)seq_len_in + (uint)q0) * (uint)cin + (uint)ic;
                acc0 += float(input[in_idx]) * float(weight[w_base + (uint)((kernel_size - 1) - r0)]);
              }
            }
            if (valid_count > 1) {
              const int q1 = (t0 + 1) / stride;
              ...
              if (q1 > 0 && q1 - 1 < seq_len_in) {
                ...
                acc1 += float(input[in_idx]) * float(weight[w_base + (uint)((stride - 1) - r1)]);
              }
              if (q1 >= 0 && q1 < seq_len_in) {
                const uint in_idx = (b * (uint)seq_len_in + (uint)q1) * (uint)cin + (uint)ic;
                acc1 += float(input[in_idx]) * float(weight[w_base + (uint)((kernel_size - 1) - r1)]);
              }
            }
            if (valid_count > 2) {
              const int q2 = (t0 + 2) / stride;
              ...
              if (q2 > 0 && q2 - 1 < seq_len_in) {
                ...
                acc2 += float(input[in_idx]) * float(weight[w_base + (uint)((stride - 1) - r2)]);
              }
              if (q2 >= 0 && q2 < seq_len_in) {
                const uint in_idx = (b * (uint)seq_len_in + (uint)q2) * (uint)cin + (uint)ic;
                acc2 += float(input[in_idx]) * float(weight[w_base + (uint)((kernel_size - 1) - r2)]);
              }
            }
            if (valid_count > 3) {
              const int q3 = (t0 + 3) / stride;
              ...
              if (q3 > 0 && q3 - 1 < seq_len_in) {
                ...
                acc3 += float(input[in_idx]) * float(weight[w_base + (uint)((stride - 1) - r3)]);
              }
              if (q3 >= 0 && q3 < seq_len_in) {
                const uint in_idx = (b * (uint)seq_len_in + (uint)q3) * (uint)cin + (uint)ic;
                acc3 += float(input[in_idx]) * float(weight[w_base + (uint)((kernel_size - 1) - r3)]);
              }
            }
          }
        } else {
          for (int k = 0; k < kernel_size; ++k) {
            ...
            if (valid_count > 0) {
              ...
              if (expanded_time0 >= 0 && expanded_time0 < seq_len_expanded && (expanded_time0 % stride) == 0) {
                const int src_time0 = expanded_time0 / stride;
                if (src_time0 >= 0 && src_time0 < seq_len_in) {
                  const uint in_idx = (b * (uint)seq_len_in + (uint)src_time0) * (uint)cin + (uint)ic;
            ...
            if (valid_count > 1) {
              ...
              if (expanded_time1 >= 0 && expanded_time1 < seq_len_expanded && (expanded_time1 % stride) == 0) {
                const int src_time1 = expanded_time1 / stride;
                if (src_time1 >= 0 && src_time1 < seq_len_in) {
                  const uint in_idx = (b * (uint)seq_len_in + (uint)src_time1) * (uint)cin + (uint)ic;
            ...
            if (valid_count > 2) {
              ...
              if (expanded_time2 >= 0 && expanded_time2 < seq_len_expanded && (expanded_time2 % stride) == 0) {
                const int src_time2 = expanded_time2 / stride;
                if (src_time2 >= 0 && src_time2 < seq_len_in) {
                  const uint in_idx = (b * (uint)seq_len_in + (uint)src_time2) * (uint)cin + (uint)ic;
            ...
            if (valid_count > 3) {
              ...
              if (expanded_time3 >= 0 && expanded_time3 < seq_len_expanded && (expanded_time3 % stride) == 0) {
                const int src_time3 = expanded_time3 / stride;
                if (src_time3 >= 0 && src_time3 < seq_len_in) {
                  const uint in_idx = (b * (uint)seq_len_in + (uint)src_time3) * (uint)cin + (uint)ic;
