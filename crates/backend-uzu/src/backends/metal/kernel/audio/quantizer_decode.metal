...
void quantizer_decode(
  ...
  for (uint k = 0; k < (uint)codebook_dim; ++k) {
    acc += float(semantic_out_proj[semantic_proj_base + k]) * float(semantic_codebook[semantic_code_base + k]);
  }
  ...
  for (int r = 0; r < residual_quantizers; ++r) {
    ...
    const uint code_base = ((uint)r * (uint)residual_cardinality + (uint)residual_token) * (uint)codebook_dim;
    const uint proj_base = ((uint)r * (uint)input_dim + d) * (uint)codebook_dim;
    for (uint k = 0; k < (uint)codebook_dim; ++k) {
      acc += float(residual_out_proj[proj_base + k]) * float(residual_codebooks[code_base + k]);
    }
  }
