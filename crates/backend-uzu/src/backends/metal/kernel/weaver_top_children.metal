...
PUBLIC KERNEL(WeaverTopChildren)(
    ...
    const device uint64_t* candidate_ids,
  ...
  const float first_logit =
      first_valid ? candidate_scores[base + first_index] + residual_logits[base + first_index] : -INFINITY;
  ...
  for (uint child = 0; child < children; ++child) {
    ...
    if (lid == 0) {
      const float winner_logit =
          winner_index == first_index ? first_logit
          : winner_index == second_index
              ? second_logit
              : as_type<float>((winner_score & 0x80000000u) ? (winner_score ^ 0x80000000u) : ~winner_score);
