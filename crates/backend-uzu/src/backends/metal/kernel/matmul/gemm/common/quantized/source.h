...
struct QuantizedSource<SignedInt4Format, true, TILES_N, TILES_K, SIMDGROUP_BLOCK_K> {
  static METAL_FUNC DeviceTensorOperand<SignedInt4Format> make(
    ...
    return DeviceTensorOperand<SignedInt4Format>{storage + (k_element_offset >> 1), row_stride_bytes};
  }
};

template <ushort TILES_N, ushort TILES_K, ushort SIMDGROUP_BLOCK_K>
struct QuantizedSource<SignedInt4Format, false, TILES_N, TILES_K, SIMDGROUP_BLOCK_K> {
      ...
      for (ushort tile_k = 0; tile_k < TILES_K; ++tile_k) {
        ...
        for (ushort thread_row = 0; thread_row < Ops::THREAD_ELEMENT_ROWS; ++thread_row) {
          ...
          if (row < simdgroup_limit_n) {
            ...
            const ushort packed =
                *reinterpret_cast<const device ushort*>(storage + int(row) * row_stride_bytes + (k_base >> 1));
...
struct QuantizedSource<SignedInt8Format, ALIGNED_N, TILES_N, TILES_K, SIMDGROUP_BLOCK_K> {
  using Ops = uzu::matmul::MxuFragmentOps<>;
  using RegisterTile = uzu::matmul::Fragment<int8_t, TILES_N, TILES_K, Ops>;
  ...
  static METAL_FUNC RegisterTile make(
    ...
    RegisterTile tile;
    auto source = uzu::matmul::fragment_source(
        reinterpret_cast<const device int8_t*>(storage) + k_element_offset,
        row_stride_bytes
    );
    if constexpr (!ALIGNED_N) {
      source = source.bounded(simdgroup_limit_n, SIMDGROUP_BLOCK_K);
    }
