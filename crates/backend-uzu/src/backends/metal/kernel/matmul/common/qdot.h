...
template <typename T, typename U, int VALUES_PER_THREAD, int BITS, bool SIGNED_W8_STORAGE = false>
METAL_FUNC U load_vector(const device T* x, thread U* x_thread) {
  using U4 = vec<U, 4>;
  const U4 inv = U4(U(1), U(1) / U(1u << BITS), U(1) / U(1u << (2u * BITS)), U(1) / U(1u << (3u * BITS)));
  U sum = 0;
  ...
  for (int i = 0; i < VALUES_PER_THREAD / 4; i++) {
    ...
    sum += v[0] + v[1] + v[2] + v[3];
    if constexpr (SIGNED_W8_STORAGE && BITS == 8) {
      x_vec4[i] = v;
    } else {
      x_vec4[i] = v * inv;
    }
...
template <typename U, int VALUES_PER_THREAD, int BITS, bool SIGNED_W8_STORAGE = false>
METAL_FUNC U qdot(const device uint8_t* w, const thread U* x_thread, U scale, U bias, U sum) {
  ...
  if constexpr (BITS == 4) {
    ...
    if constexpr (SIGNED_W8_STORAGE) {
      ...
      METAL_PRAGMA_UNROLL
      for (int i = 0; i < (VALUES_PER_THREAD / 4); ++i) {
        const U4 weight_vec4 = U4(*reinterpret_cast<const device char4*>(signed_weights + 4 * i));
        accumulator += dot(x_vec4[i], weight_vec4);
      }
    } else {
...
METAL_FUNC U qdot_safe(const device uint8_t* w, const thread U* x_thread, U scale, U bias, U sum, int N) {
  ...
  if constexpr (BITS == 4) {
    ...
    for (int i = 0; i < N; i++) {
      accumulator += x_thread[i] * (SIGNED_W8_STORAGE ? U(signed_w[i]) : U(w[i]));
    }
  }
