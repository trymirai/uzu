...
auto gemm_loop(
  ...
  if constexpr (!aligned_k) {
    ...
    for (int inner_k = 0; inner_k < remaining_k; inner_k += SIMDGROUP_BLOCK_K) {
      ...
      const short2 left_limits =
          transpose_a ? short2(simdgroup_limit_m, safe_k) : short2(safe_k, simdgroup_limit_m);
      const short2 right_limits =
          transpose_b ? short2(safe_k, simdgroup_limit_n) : short2(simdgroup_limit_n, safe_k);
