...
// Upper bound for the threadgroup reduction scratch (in floats). Max config:
// tg_simd_cols=16, output_rows_per_tg=16, thread_out_rows=4 → 16 * (16 + 4) =
// 320. Unused when tg_simd_cols == 1.
#define GEMV_MAX_THREADGROUP_MEMORY 320

namespace uzu {
namespace gemv {

// Full-precision GEMV core: a tile-driven simdgroup-reduction matrix-vector
// product. Dense weights laid out [out_vec_size, in_vec_size]; the tunable tile
// (tg/sg/thread counts) arrives as the `gemv_tiling` function constant. The
// group-quantized path lives in the separate `qmv_fast` kernel, which the GPU
// prefers for bandwidth-bound packed weights.
//
// Grid axes: `threadgroup_position.x` = output block on N, `.y` = batch row on
// M. Threadgroup geometry: 32 lanes × 8 simdgroups × 1 (set in gemv.metal).
// Epilogue: optional scale → optional accumulate → optional bias.
template <typename T>
struct GemvCore {
