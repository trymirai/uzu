...
// Unified GEMV core: a single simdgroup-reduction matrix-vector implementation
// that handles both full-precision (BITS == 0) and group-quantized weights,
// selected at compile time. Mirrors gemm's *MmaCore structs — KERNEL(Gemv) is a
// thin dispatch over this. The two paths keep their own proven inner loops; only
// the entry point and dispatch are shared.
