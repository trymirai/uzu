...
struct BSource {
  static METAL_FUNC void accumulate(
    ...
    if constexpr (B_PROLOGUE == GemmBPrologueKind::FullPrecision) {
      ...
      const device AT* in = a + batch_idx * in_vec_size + simd_lane * values_per_thread + k_start;

      uint k = k_start;
      for (; k + block_size <= in_vec_size; k += k_stride) {
        float4 xv = static_cast<float4>(*reinterpret_cast<const device I4*>(in));
        result[0] += dot(static_cast<float4>(*reinterpret_cast<const device W4*>(w)), xv);
        result[1] += dot(static_cast<float4>(*reinterpret_cast<const device W4*>(w + in_vec_size)), xv);
        result[2] += dot(static_cast<float4>(*reinterpret_cast<const device W4*>(w + 2 * in_vec_size)), xv);
        result[3] += dot(static_cast<float4>(*reinterpret_cast<const device W4*>(w + 3 * in_vec_size)), xv);
        w += k_stride;
        in += k_stride;
      }
      ...
      const device AT* in = a + batch_idx * in_vec_size + simd_lane * values_per_thread;
      ...
      if constexpr (!INPUT_ALIGNED) {
        ...
        const int remaining = (k + thread_offset < in_vec_size) ? min(static_cast<int>(in_vec_size - k - thread_offset),
                                                                ...
                                                                : 0;
        if (remaining > 0) {
          U sum = load_vector_safe<AT, U, values_per_thread>(in, x_thread, remaining);
          ...
          U offset[4];
          prep.load(scale, offset);
          result[0] += qdot_safe<U, values_per_thread, BITS>(wl0, x_thread, scale[0], offset[0], sum, remaining);
          result[1] += qdot_safe<U, values_per_thread, BITS>(wl1, x_thread, scale[1], offset[1], sum, remaining);
          result[2] += qdot_safe<U, values_per_thread, BITS>(wl2, x_thread, scale[2], offset[2], sum, remaining);
          result[3] += qdot_safe<U, values_per_thread, BITS>(wl3, x_thread, scale[3], offset[3], sum, remaining);
        }
      }
    }
  }
