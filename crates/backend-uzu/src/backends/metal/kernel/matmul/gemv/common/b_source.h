...
struct LloydMaxBSource {
  static METAL_FUNC void accumulate(
    ...
    QuantRowOffsets<BT, U, GemmBPrologueKind::LloydMaxDequant, BITS> prep;
    prep.group_stride = in_vec_size_g;
    const uint g_offset = simd_lane / scale_step_per_thread;
    prep.scales = scales + out_row * in_vec_size_g + g_offset;
    const uint bias_stride = (in_vec_size_g + 1) / 2;
    const device uint8_t* bias_codes = bias_indices + out_row * bias_stride + g_offset / 2;
    const bool bias_high_nibble = (g_offset & 1);
