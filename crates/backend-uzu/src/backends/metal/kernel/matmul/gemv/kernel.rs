...
use crate::{
    backends::{
        ...
        metal::{
            ...
            kernel::{GemvLloydMaxMetalKernel, GemvMetalKernel},
        },
    },
    data_type::DataType,
};
...
pub(crate) struct GemvDispatch {
    ...
    lloyd_max_pipelines: HashMap<GemvSpecialization, GemvLloydMaxMetalKernel>,
}
...
impl GemvDispatch {
    ...
    fn get_or_create_lloyd_max(
        ...
        context: &MetalContext,
        specialization: GemvSpecialization,
    ) -> Result<&GemvLloydMaxMetalKernel, MatmulError<Metal>> {
        match self.lloyd_max_pipelines.entry(specialization) {
            Entry::Occupied(entry) => Ok(entry.into_mut()),
            Entry::Vacant(entry) => {
                let kernel = GemvLloydMaxMetalKernel::new(
                    context,
                    self.input_data_type,
                    self.weights_data_type,
                    self.output_data_type,
