...
// Dedicated quantized GEMV kernel matching the baseline `qmv_fast` signature
// for perf — direct THREADS/GROUPS params, separate `const constant uint&`
// scalars, single threadgroup scratch — plus the scale/accumulate/bias
// transforms the unified GEMV gained (driven via `output_transform`). Mirrors
// the layout the GPU likes for bandwidth-bound packed-weight gemv: 8
// simdgroups × 32 lanes, 4 outputs per simdgroup, lane-0 writes.
template <
    typename T,
...
KERNEL(QmvFast)(
  ...
  if (remaining > 0) {
    ...
    if constexpr (B_PROLOGUE == GemmBPrologueKind::ScaleBiasDequant) {
      ...
      result[0] += qdot_safe<U, values_per_thread, BITS>(
          wl0, x_thread, s0,
          -s0 * static_cast<U>(zp_nibbles.x), sum, remaining);
      result[1] += qdot_safe<U, values_per_thread, BITS>(
          wl1, x_thread, s1,
          -s1 * static_cast<U>(zp_nibbles.y), sum, remaining);
      result[2] += qdot_safe<U, values_per_thread, BITS>(
          wl2, x_thread, s2,
          -s2 * static_cast<U>(zp_nibbles.z), sum, remaining);
      result[3] += qdot_safe<U, values_per_thread, BITS>(
          wl3, x_thread, s3,
          -s3 * static_cast<U>(zp_nibbles.w), sum, remaining);
    }
  }
