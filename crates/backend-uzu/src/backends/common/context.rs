...
pub trait Context: Sized {
    ...
    /// Block until every sparse map/unmap operation issued so far has been
    /// committed on the GPU. Must be called before any kernel on the main
    /// command queue can read or write pages that were just mapped, because
    /// sparse mapping ops run on a separate (MTL4) queue.
