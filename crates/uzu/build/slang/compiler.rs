use std::{collections::HashMap, env, ffi::CString, fs, iter::once, path::PathBuf};
...
use shader_slang::{
    CompileTarget, CompilerOptions, Downcast, GlobalSession, OptimizationLevel, Session, SessionDesc, TargetDesc,
};
...
use crate::{
    ...
    debug_log,
    slang::{bindgen::bindgen, reflection::SlangKernelInfo, slang_api, wrapper},
};
...
impl BlockingSlangCompiler {
    fn new(fields: SlangCompilerFields) -> anyhow::Result<Self> {
        ...
        let search_path =
            CString::new(fields.src_dir.to_string_lossy().as_bytes()).context("cannot convert src_dir to CString")?;
        ...
        let session_desc =
            SessionDesc::default().options(&session_options).search_paths(&search_paths).targets(&targets);
    ...
    fn compile(
        ...
        let source_name =
            source_file.with_extension("").file_name().context("source path has no file name")?.to_os_string();
        let source_dir = source_file.parent().context("cannot get source file's directory")?;
        ...
        let source_file_pretty = source_file.file_name().unwrap().to_str().unwrap();
        debug_log!("compile start: {source_file_pretty}");

        let buildsystem_hash: [u8; blake3::OUT_LEN] =
            caching::build_system_hash().context("cannot get build system hash")?.as_bytes().clone();

        if let Ok(dephashes_contents) = fs::read(&dephashes_file)
            && let Ok(cached) = serde_json::from_slice::<Dephashes>(&dephashes_contents)
        ...
        let source_file_str = source_file.to_str().context("cannot convert the path to &str")?;

        let shader_load_result =
            slang_api::load_module(&self.session, source_file_str).context("cannot load shader module")?;
        ...
        if !kernels.is_empty() {
            let wrapper_blocks = kernels
                .iter()
                .map(|kernel| {
                    wrapper::generate_wrappers(kernel, &shader_load_result.module)
            ...
            let wrapper_contents = once(format!("import {source_file_str:?};")).chain(wrapper_blocks).join("\n\n");

            fs::write(&wrapper_file, wrapper_contents.as_bytes()).context("cannot write wrapper file")?;

            let wrapper_file_str = wrapper_file.to_str().context("cannot convert the path to &str")?;

            let wrapper_load_result =
                slang_api::load_module(&self.session, wrapper_file_str).context("cannot load wrapper module")?;
            ...
            let compiled_module = wrapper_load_result.module.downcast().link().context("cannot link wrapper module")?;

            let compiled_blob = compiled_module.target_code(0).context("cannot get target code")?;

            fs::write(&object_file, compiled_blob.as_slice()).context("cannot write object file")?;

            let bindgen_blocks = kernels
                .iter()
                .map(|kernel| {
                    bindgen(kernel).with_context(|| format!("cannot generate bindings for kernel '{}'", kernel.name()))
                })
                .collect::<anyhow::Result<Vec<_>>>()?;

            let bindgen_contents = bindgen_blocks.join("\n\n");

            fs::write(&bindgen_file, bindgen_contents.as_bytes()).context("cannot write bindgen file")?;
        }
...
impl SlangCompiler {
    pub fn new() -> anyhow::Result<Self> {
        ...
        let out_dir = PathBuf::from(env::var("OUT_DIR").context("missing OUT_DIR")?).join("vulkan");
    ...
    where
        T: Send + 'static,
        F: FnOnce(&mut BlockingSlangCompiler) -> anyhow::Result<T> + Send + 'static,
    {
        let blocking_compiler = if let Some(bc) = self.blocking_compiler_pool.lock().await.pop() {
            bc
        } else {
