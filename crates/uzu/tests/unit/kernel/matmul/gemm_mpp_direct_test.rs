...
fn test_mpp_direct_correctness(
    batch_dim: usize,
    input_dim: usize,
    output_dim: usize,
    ab_scale: f32,
    ...
    if !metal_context.device.supports_mxu() {
        eprintln!("Skipping MPP Direct test: device does not support MXU");
        return;
    }

    let input = get_test_data(batch_dim, input_dim, output_dim, ab_scale, accumulate);
    let expected = get_cpu_output(&input);
    let actual = get_mpp_direct_output(&metal_context, &mut matmul_kernel, &input);
    assert_eq_float(&expected, &actual, tolerance, "GEMM_MPP_DIRECT vs CPU reference");
}

#[uzu_test]
fn test_bf16_aligned() {
    test_mpp_direct_correctness(64, 2048, 2048, 1.0, false, 1.0);
}

#[uzu_test]
fn test_bf16_unaligned_m() {
    test_mpp_direct_correctness(33, 2048, 2048, 1.0, false, 1.0);
}

#[uzu_test]
fn test_bf16_unaligned_n() {
    test_mpp_direct_correctness(64, 2048, 33, 1.0, false, 1.0);
}

#[uzu_test]
fn test_bf16_large() {
    test_mpp_direct_correctness(128, 4096, 14336, 1.0, false, 2.0);
}

#[uzu_test]
fn test_bf16_small_batch() {
    test_mpp_direct_correctness(1, 2048, 2048, 1.0, false, 1.0);
}

#[uzu_test]
fn test_bf16_ab_scale() {
    test_mpp_direct_correctness(64, 2048, 2048, 0.5, false, 1.0);
}
...
#[uzu_bench]
fn bench_gemm_mpp_direct(criterion: &mut Criterion) {
    ...
    let mut benchmark_group = criterion.benchmark_group(format!(
        "{}/Kernel/Matmul/GEMM_MPP_DIRECT",
