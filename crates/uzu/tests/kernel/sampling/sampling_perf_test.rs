...
//
// Measures GPU execution time for fused stochastic sampling.
...
fn perf_fused_128k_vocab() {
    for_each_non_cpu_backend!(|B| {
        ...
        let fused_times = time_encode::<B, _>(
            &context,
            &logits,
            &seeds,
            BATCH,
        ...
        println!("  fused: {}", summarize(&fused_times));
    });
}
...
fn perf_batch1_128k_vocab() {
    for_each_non_cpu_backend!(|B| {
        ...
        let times = time_encode::<B, _>(&context, &logits, &seeds, BATCH, VOCAB, WARMUP, RUNS, |lb, sb, ob, cb| {
            kernel
                .encode(
                    lb,
                    sb,
                    ...
                    0,
                    ob,
                    SamplingMethod { temperature: Some(0.6), top_k: Some(20), top_p: Some(0.95), min_p: None },
                    BATCH,
                    VOCAB,
                    cb,
                )
        ...
        if times.is_empty() {
            println!("[{}] batch={BATCH} vocab={VOCAB}: (no GPU timing)", std::any::type_name::<B>());
        } else {
            let mean = times.iter().sum::<f64>() / times.len() as f64;
            let min = times.iter().cloned().fold(f64::INFINITY, f64::min);
            println!("[{}] batch={BATCH} vocab={VOCAB}: mean={mean:.3}ms  min={min:.3}ms", std::any::type_name::<B>());
        }
    });
}
