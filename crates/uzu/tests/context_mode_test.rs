...
use uzu::session::{
    ...
    parameter::{AsyncBatchSize, ContextLength, ContextMode, PrefillStepSize, SamplingPolicy, SamplingSeed},
...
fn request(
    ...
    session
        .run(
            ...
            RunConfig::new(tokens_limit, false, SamplingPolicy::Default, None),
            None::<fn(Output) -> bool>,
        )
        .unwrap()
        .text
...
fn test_context_mode_static() {
    ...
    let response_age = request(&mut session, format!("What is {}'s age?", parameter_name), tokens_limit);
    let response_city = request(&mut session, format!("What is {}'s city?", parameter_name), tokens_limit);

    println!("Occupation: {}", response_occupation);
    println!("Age: {}", response_age);
    println!("City: {}", response_city);

    assert!(response_occupation.to_lowercase().contains(parameter_occupation.to_lowercase().as_str()));
    assert!(response_age.to_lowercase().contains(parameter_age.to_lowercase().as_str()));
...
fn test_context_mode_dynamic() {
    ...
    update(&mut session, "Update: Name: Dave. Occupation: Doctor. Age: 30.".to_string());
    ...
    update(&mut session, "Update: Name: Dave. Occupation: Doctor. Age: 31.".to_string());
    ...
    let answer_4 = ask(&mut session, "What is Dave's current occupation?".to_string());
