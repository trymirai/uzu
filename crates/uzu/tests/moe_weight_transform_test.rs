...
fn test_weight_transform_simple() {
    ...
    let fused: Vec<f16> = (0..num_experts * d_model * d_ff * 2)
    ...
    let output_size = (num_experts * d_ff * d_model * 2) as u64;
    ...
    let threadgroups = metal::MTLSize::new(num_tg_x, num_tg_y, num_experts as u64);
...
#[test]
fn test_weight_transform_bigger_model() {
    ...
    let fused: Vec<f16> = {
        let mut vec = vec![f16::ZERO; num_experts * 2 * d_ff * d_model];
        let mut rng = StdRng::seed_from_u64(42);
        for item in vec.iter_mut() {
            *item = f16::from_f32(rng.gen_range(-1.0f32..1.0f32));
        }
        vec
    };
    
    let cpu_w1: Vec<f16> = {
        let mut result = Vec::with_capacity(num_experts * d_ff * d_model);
        for e in 0..num_experts {
            for r in 0..d_ff {
                for k in 0..d_model {
                    let src_idx = e * 2 * d_ff * d_model + k * 2 * d_ff + r;
                    result.push(fused[src_idx]);
                }
            }
        }
        result
    ...
    let cpu_w3: Vec<f16> = {
        ...
        for e in 0..num_experts {
            for r in 0..d_ff {
                for k in 0..d_model {
                    let src_idx = e * 2 * d_ff * d_model + k * 2 * d_ff + r + d_ff;
                    result.push(fused[src_idx]);
                }
            }
        }
    ...
    let device = metal::Device::system_default().unwrap();
    let command_queue = device.new_command_queue();
    let ctx = MTLContext::new(device, command_queue).unwrap();
    
    let fused_buf = ctx.device.new_buffer_with_data(
        fused.as_ptr() as *const _,
        (fused.len() * std::mem::size_of::<f16>()) as u64,
        metal::MTLResourceOptions::StorageModeShared,
    );
    
    let w1_buf = ctx.device.new_buffer(
        (num_experts * d_ff * d_model * std::mem::size_of::<f16>()) as u64,
        metal::MTLResourceOptions::StorageModeShared,
    );
    let w3_buf = ctx.device.new_buffer(
        (num_experts * d_ff * d_model * std::mem::size_of::<f16>()) as u64,
        metal::MTLResourceOptions::StorageModeShared,
    );
    ...
    let num_tg_x = ((d_model + 15) / 16) as u64;
    let num_tg_y = ((d_ff + 15) / 16) as u64;
    let num_tg_z = num_experts as u64;
    let threadgroups =
    ...
    for expert_idx in 0..num_experts {
        for ff_idx in 0..d_ff {
            for d_idx in 0..d_model {
                let w1_idx = (expert_idx * d_ff + ff_idx) * d_model + d_idx;
                
                assert_eq!(
                    w1_slice[w1_idx], cpu_w1[w1_idx],
                    "W1 mismatch at e={}, ff={}, d={}",
                    expert_idx, ff_idx, d_idx
                );
                
                assert_eq!(
                    w3_slice[w1_idx], cpu_w3[w1_idx],
                    "W3 mismatch at e={}, ff={}, d={}",
                    expert_idx, ff_idx, d_idx
                );
            }
        }
