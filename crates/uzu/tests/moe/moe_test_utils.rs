...
/// Computes: out[t, e] = input[t, d_model] @ weight[e, d_model]^T + bias[e]
/// This matches the GPU shader which:
/// 1. Loads inputs/weights as vec4 (4 elements at a time)
/// 2. Uses FMA instruction: accum4 = fma(wv, xv, accum4)
/// 3. Sums the 4-vector accumulator
/// 4. Stores logits in threadgroup as f32
...
/// When renorm=false, the output values are quantized to bf16 to match GPU behavior.
pub fn cpu_topk_select_f32(
    ...
    for token in 0..t {
        ...
        if renorm {
            ...
            // When renorm=false, GPU stores raw logits as bfloat in output buffer
            // Match this behavior by quantizing to bf16 and back
            for kk in 0..k {
