...
impl<B: Backend> DeltaNetMixer<B> {
    ...
    pub(crate) fn new(
        ...
        let norm_weight = resolve_subtree(&mixer_tree, &["norm", "inner_norm"]).leaf_array("scales").unwrap().clone();
    ...
    fn run_conv_scan(
        ...
        let padded_cell = state.conv_padded_buffer().expect("Missing conv padded buffer");
        let padded = padded_cell.borrow();
    ...
    pub(crate) fn encode(
        ...
        if active_suffix_length == 1 {
            self.run_conv_update(state, command_buffer);
            self.run_delta_rule(state, command_buffer);
        } else {
            self.run_conv_scan(state, command_buffer, active_suffix_length);
            self.run_delta_rule_prefill(state, command_buffer, active_suffix_length);
        }
