...
impl<B: Backend> FullPrecisionLinear<B> {
    pub fn new(
        ...
        let (weights, bias) = checked_linear_arrays(precision, input_dim, output_dim, parameter_tree)?;
        let kernel = <B::Kernels as MatmulKernels>::FullPrecisionMatmulKernel::new(context, precision)?;
        Ok(Self {
            kernel: RefCell::new(kernel),
            bias_buffer: bias.map(|bias| bias.buffer()),
...
fn checked_linear_arrays<B: Backend>(
    precision: DataType,
    input_dim: usize,
    output_dim: usize,
    parameter_tree: &ParameterTree<B::Context>,
) -> Result<(Array<B>, Option<Array<B>>), FullPrecisionLinearError<B>> {
    ...
    let bias = match parameter_tree.leaf_array("biases") {
        Ok(bias) => {
        ...
        Err(error) => return Err(FullPrecisionLinearError::ParameterError(error)),
    };
...
impl<B: Backend> Linear<B> for FullPrecisionLinear<B> {
    fn encode(
        ...
        let bias_borrow = self.bias_buffer.as_ref().map(|buffer| buffer.borrow());
