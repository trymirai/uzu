...
struct PendingStreamingChunk {
    submission_decode_duration: std::time::Duration,
    ready_frames: usize,
...
fn audio_decode_streaming_mode(config: &TtsRunConfig) -> AudioDecodeStreamingMode {
...
impl TokenDecoderRunner {
        ...
        for pass in 0..followup_count {
            ...
            self.command_buffer.borrow_mut().with_compute_encoder(|encoder| {
                if pass + 1 < followup_count {
                    self.token_copy_sampled.encode(sampling_output_buffer.clone(), token_ids_buffer.clone(), encoder);
                }
    ...
    fn submit_and_wait_current_command_buffer(&mut self) -> Result<(), Error> {
        ...
        self.command_buffer.borrow().wait_until_completed().map_err(|err| Error::CommandBufferFailed(Box::new(err)))?;
...
mod tests {
    ...
    use crate::audio::{AudioPcmBatch, AudioTokenPacking};
