...
impl BucketedPool {
    ...
    fn return_buffer(
        ...
        let bucket_size = next_power_of_two(actual_size);
...
unsafe impl Send for BucketedAllocator {}
unsafe impl Sync for BucketedAllocator {}
...
impl MetalAllocator for BucketedAllocator {
    ...
    fn alloc_scratch(&self, size: usize) -> Result<AllocatedBuffer, AllocError> {
        ...
        if let Ok(mut pool) = self.pool.lock() {
            if let Some(buffer) = pool.find_buffer(aligned_size) {
    ...
    fn free(&self, buffer: AllocatedBuffer) {
        ...
        match buffer.lifetime {
            ...
            BufferLifetime::Scratch => {
                if let Ok(mut pool) = self.pool.lock() {
                    pool.return_buffer(buffer.buffer, buffer.size);
                } else {
                    drop(buffer);
                }
            }
