...
impl ForwardPassState {
    pub fn new(
        ...
        // Token Positions (i32)
        // --------------------
        let token_positions_refcell = if use_async_positions {
            let (buffer, offset) = async_positions.expect(
                "async_positions required when use_async_positions=true",
            );
