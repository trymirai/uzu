...
template [[host_name("apply_sigmoid_f16")]]
[[kernel]] void apply_sigmoid<half>(
    const device half* input [[buffer(0)]],
    device half* output [[buffer(1)]],
    constant int& total_elements [[buffer(2)]],
    uint tid [[thread_position_in_grid]]);

template [[host_name("apply_sigmoid_f32")]]
[[kernel]] void apply_sigmoid<float>(
    const device float* input [[buffer(0)]],
    device float* output [[buffer(1)]],
    constant int& total_elements [[buffer(2)]],
    uint tid [[thread_position_in_grid]]);

template [[host_name("apply_sigmoid_bf16")]]
[[kernel]] void apply_sigmoid<bfloat>(
