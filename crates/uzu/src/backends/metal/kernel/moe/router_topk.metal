    ...
    threadgroup float* shared_best_val)
{
    ...
    const device Vec4T* x_vec =
        input + (ulong)token_idx * (ulong)vecs;
    ...
    for (uint row = simdgroup_idx; row < E; row += simdgroups_per_tg) {
        const device Vec4T* w_vec =
            weight + (ulong)row * (ulong)vecs;
    ...
    for (uint sel = 0; sel < effective_k; ++sel) {
        ...
        float max_val =
            threadgroup_cooperative_reduce_max<THREADS_PER_TG>(
                local_best,
                reduce_tmp,
                lid);

        uint candidate_id =
            (local_best == max_val) ? local_idx : 0xFFFFFFFFu;
        uint best_idx =
            threadgroup_cooperative_reduce_min<THREADS_PER_TG>(
                candidate_id,
                reduce_tmp_u,
                lid);
...
{ \
    ...
    moe_router_topk_impl<VEC4, SCALAR, PROB>( \
        input, weight, bias, topk_ids, topk_probs, \
        T, d_model, E, K, renorm, tgpig, lid, simd_lane, simdgroup_idx, simdgroups_per_tg, \
        x_cache, logits_shared, idx_shared, reduce_tmp, reduce_tmp_u, \
        &shared_best_idx_mem, &shared_best_val_mem); \
}
