...
#define THREADS_PER_TG 256 // 8 simdgroups
#define SIMD_WIDTH 32
#define NUM_SG (THREADS_PER_TG / SIMD_WIDTH)

constant uint MAX_EXPERTS = 512;
constant uint MAX_TOPK = 128;
constant float NEG_INF = -INFINITY;

template <typename ScalarT, typename ProbT>
VARIANTS(ScalarT, float, half, bfloat)
VARIANTS(ProbT, float, half, bfloat)
KERNEL(MoeRouterTopK)(
  ...
  const uint simd_lane = lid % SIMD_WIDTH;
  const uint simdgroup_idx = lid / SIMD_WIDTH;
  const uint simdgroups_per_tg = NUM_SG;

  const device ScalarT* x_vec = input + (ulong)token_idx * (ulong)d_model;
  ...
  for (uint row = simdgroup_idx; row < e; row += simdgroups_per_tg) {
    const device ScalarT* w_vec = weight + (ulong)row * (ulong)d_model;
