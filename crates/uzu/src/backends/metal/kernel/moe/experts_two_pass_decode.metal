...
PUBLIC KERNEL(MoeExpertsDecodePassA)(
  ...
  const uint h_idx = h_block_idx * 4 + simd.threadgroup_index;
  if (h_idx >= d_ff)
    return;
  ...
  for (uint i = 0; i < vec_iters; ++i) {
    uint base_idx = i * 128 + simd.simdgroup_index * 4;
  ...
  uint leftover_start = vec_iters * 128 + simd.simdgroup_index;
...
PUBLIC KERNEL(MoeExpertsDecodeDownFused2D)(
  ...
  const uint my_col = tgpig_x * SIMDGROUPS_PER_TG + simd.threadgroup_index;
  if (my_col >= d_model)
    return;
  ...
  for (uint iter = 0; iter < k_vec_iters; ++iter) {
    const uint k_base = iter * (8 * THREADS_PER_SIMD) + simd.simdgroup_index;
  ...
  for (uint iter = k_vec_iters * 8; iter < k_iters; ++iter) {
    const uint k = iter * THREADS_PER_SIMD + simd.simdgroup_index;
    acc =
  ...
  if (leftover_start + simd.simdgroup_index < d_ff) {
    const uint k = leftover_start + simd.simdgroup_index;
    acc =
        fma(AccumT(hidden[hidden_base + k]),
            AccumT(w2_all[w2_col_base + k]),
            acc);
