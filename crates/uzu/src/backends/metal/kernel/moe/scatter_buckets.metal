...
inline void moe_scatter_buckets_impl(
  ...
  for (uint e0 = 0; e0 < E; e0 += TILE_E) {
        ...
        if (eid >= 0) {
          ...
          if (ue >= e0 && ue < e0 + tile_e) {
            ...
            atomic_fetch_add_explicit(
                &sg_counts[simd.threadgroup_index * TILE_E + te],
                1u,
                memory_order_relaxed
            );
          }
    ...
    for (uint step = 0; step < simd.simdgroup_size; ++step) {
      if (simd.simdgroup_index == step) {
          ...
          for (uint k = 0; k < K; ++k) {
            ...
            if (eid >= 0) {
              ...
              if (ue >= e0 && ue < e0 + tile_e) {
                ...
                const uint base_idx_block =
                    (block_id * num_tiles + tile_id) * TILE_E + te;
                const uint idx_sg_te = simd.threadgroup_index * TILE_E + te;
