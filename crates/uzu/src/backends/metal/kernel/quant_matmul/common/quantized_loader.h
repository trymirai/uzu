...
struct QuantizedBlockLoaderAffineBias {
  static_assert(
      BLOCK_COLS % group_size == 0 || group_size % BLOCK_COLS == 0,
      "BLOCK_COLS and group_size must be multiples of each other"
  );
  ...
  void load_unchecked() const {
    ...
    if constexpr (BLOCK_COLS <= group_size) {
      T scale = *scales;
      T bias = *biases;
      ...
      for (int i = 0; i < read_count; i++) {
        short actual_col = (block_col + i) * pack_factor;
        short group_index = actual_col / group_size;
  ...
  void next() {
    ...
    if (reduction_dim == 1) {
      if constexpr (BLOCK_COLS > group_size) {
        constexpr short groups_per_tile = BLOCK_COLS / group_size;
        scales += groups_per_tile;
        biases += groups_per_tile;
      } else if constexpr (group_steps > 1) {
        group_step_counter++;
...
struct QuantizedBlockLoaderZeroPoint {
  ...
  inline void scale_and_bias_for_group_index(
      int group_index,
      thread T& out_scale,
      thread T& out_bias
  ) const {
    T scale_value = scales_row_start[group_index];
    uint zero_point_numeric;
    if (bits == 4) {
      const device uint8_t* zero_point_pointer =
          zero_points_row_start + (group_index >> 1);
  ...
  void load_unchecked() const {
    ...
    if constexpr (BLOCK_COLS <= group_size) {
      ...
      int base_group_index = reduction_dim == 1
          ? (int)(scales - scales_row_start)
          : (k_base / group_size);
      for (int i = 0; i < read_count; i++) {
        short actual_col = (block_col + i) * pack_factor;
        short group_offset = actual_col / group_size;
        T scale;
        T bias;
        scale_and_bias_for_group_index(base_group_index + group_offset, scale, bias);
        dequantize<T, pack_factor, bits>(
            source + i * bytes_per_pack,
            scale,
            bias,
  ...
  void load_checked(short2 source_tile_dimensions) const {
    ...
    if (reduction_dim == 1) {
      ...
      if constexpr (BLOCK_COLS <= group_size) {
        ...
        int base_group_index = (int)(scales - scales_row_start);
        for (int i = 0; i < read_count; i++) {
          int pack_index = block_col + i;
          if (pack_index < valid_packs) {
            short actual_col = (block_col + i) * pack_factor;
    ...
    if constexpr (BLOCK_COLS <= group_size) {
      ...
      int base_group_index = k_base / group_size;
      for (int i = 0; i < read_count; i++) {
        short actual_col = (block_col + i) * pack_factor;
        short group_offset = actual_col / group_size;
        T scale;
