...
METAL_FUNC void gemv_entry(
    ...
    const constant int& bias_stride [[buffer(14)]],
    threadgroup
    typename GEMVKernel<T, BM, BN, SM, SN, TM, TN, kDoAxpby>::acc_type*
        tgp_memory,
    uint3 tid [[threadgroup_position_in_grid]],
    uint3 lid [[thread_position_in_threadgroup]],
    uint simd_gid [[simdgroup_index_in_threadgroup]],
...
METAL_FUNC void gemv_t_entry(
    ...
    const constant int& bias_stride [[buffer(14)]],
    threadgroup
    typename GEMVTKernel<T, BM, BN, SM, SN, TM, TN, kDoAxpby>::acc_type*
        tgp_memory,
    uint3 tid [[threadgroup_position_in_grid]],
    uint3 lid [[thread_position_in_threadgroup]],
    uint simd_gid [[simdgroup_index_in_threadgroup]],
...
#define instantiate_gemv_helper(                                                                    \
  ...
  void                                                                                              \
  gemv_inst_##name##_bm##bm##_bn##bn##_sm##sm##_sn##sn##_tm##tm##_tn##tn##_nc##nc##_axpby##axpby(   \
  ...
  [[host_name(                                                                                      \
      "gemv_t_" #name "_bm" #bm "_bn" #bn "_sm" #sm "_sn" #sn "_tm" #tm                             \
      "_tn" #tn "_nc" #nc "_axpby" #axpby                                                           \
  )]] [[kernel, max_total_threads_per_threadgroup(bm * bn * 32)]]                                   \
  void                                                                                              \
  gemv_t_inst_##name##_bm##bm##_bn##bn##_sm##sm##_sn##sn##_tm##tm##_tn##tn##_nc##nc##_axpby##axpby( \
      const device itype* mat [[buffer(0)]],                                                        \
      const device itype* in_vec [[buffer(1)]],                                                     \
      const device itype* bias [[buffer(2)]],                                                       \
      device itype* out_vec [[buffer(3)]],                                                          \
