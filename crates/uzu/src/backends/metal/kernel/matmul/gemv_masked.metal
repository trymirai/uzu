...
METAL_FUNC void gemv_masked_impl(
  ...
  for (uint idx = simd_lid; idx < k; idx += 32) {
    ...
    if (op_on) {
      acc += static_cast<AccT>(row_ptr[idx]) * static_cast<AccT>(vec_ptr[idx]);
    }
  }
...
// Masked/gather GEMV kernels are stubbed out (not used). Keeping file empty to
// avoid shader compilation errors.
