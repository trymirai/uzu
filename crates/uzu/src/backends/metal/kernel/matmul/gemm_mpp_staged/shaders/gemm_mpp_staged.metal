...
#include "cooperative_tensor_gemm.h"
...
template <
    ...
    short SUBTILE_COLS,
    short MATMUL_K_STEP>
METAL_FUNC void gemm_mpp_staged_impl(
...
#define GEMM_MPP_STAGED_DISPATCH(T, BLOCK_ROWS, BLOCK_COLS, SIMDGROUPS_PER_ROW, SIMDGROUPS_PER_COLUMN) \
  if (block_rows == BLOCK_ROWS && block_cols == BLOCK_COLS && \
    ...
    gemm_mpp_staged_impl<T, BLOCK_ROWS, BLOCK_COLS, SIMDGROUPS_PER_ROW, SIMDGROUPS_PER_COLUMN, 16, 32, 16>( \
...
KERNEL(MatmulGemmMppStaged)(
  ...
  GEMM_MPP_STAGED_DISPATCH(T, 64, 64, 2, 2)
  GEMM_MPP_STAGED_DISPATCH(T, 32, 64, 2, 2)
  GEMM_MPP_STAGED_DISPATCH(T, 64, 32, 4, 1)
}

#undef GEMM_MPP_STAGED_DISPATCH
// clang-format on
