...
use super::{
    super::dsl::{MatmulGemmMetalKernel, MatmulGemmMppMetalKernel, MatmulGemvMetalKernel, TensorAddBiasMetalKernel},
    gemm, gemm_mpp, gemv,
};
use crate::{
    DataType,
    backends::{
        common::{
            ...
            kernel::{
                ...
                matmul::{MatmulArguments, MatmulError, MatmulKernel as MatmulKernelTrait},
            },
        },
        metal::{
            GpuTier, Metal, MetalDeviceCapabilities, command_buffer::MetalCommandBufferEncoding, context::MetalContext,
        },
    },
};
...
impl MatmulMetalKernel {
    pub fn encode_gemv(
        ...
        let specialization =
            gemv::Specialization::select(input_dim, effective_output_dimension, apply_output_scale_and_accumulate);

        let kernel = self.get_or_create_gemv(context, specialization).expect("Failed to create GEMV kernel");
        ...
        let output_source: Option<(&Retained<ProtocolObject<dyn MTLBuffer>>, usize)> =
    ...
    pub fn encode_gemm_mpp(
        ...
        let (params, threadgroups) =
            GEMMParams::with_grid(m, n, k, specialization.block_rows, specialization.block_cols, 0, 0);

        let kernel = self.get_or_create_gemm_mpp(context, specialization).expect("Failed to create GEMM MPP kernel");
    ...
    pub fn encode_gemm(
        ...
        let kernel = self.get_or_create_gemm(context, specialization).expect("Failed to create GEMM kernel");
...
impl MatmulMetalKernel {
    ...
    fn encode_bias_add(
        &mut self,
        context: &MetalContext,
        bias: Option<&Retained<ProtocolObject<dyn MTLBuffer>>>,
        output: &mut Retained<ProtocolObject<dyn MTLBuffer>>,
        batch: usize,
        output_dim: usize,
        command_buffer: &mut MetalCommandBufferEncoding,
    ) {
        ...
        if self.bias_add.is_none() {
            self.bias_add = Some(
                TensorAddBiasMetalKernel::new(context, self.data_type, true).expect("Failed to create bias add kernel"),
            );
        }
...
fn default_gemv_max_batch(device_info: &MetalDeviceCapabilities) -> i32 {
...
fn gemv_max_batch(device_info: &MetalDeviceCapabilities) -> i32 {
...
mod tests {
    ...
    use crate::backends::metal::{DeviceGeneration, GpuTier, MetalDeviceCapabilities};
