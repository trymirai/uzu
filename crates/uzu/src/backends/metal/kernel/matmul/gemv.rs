...
use super::{super::mlp::MlpActivationType, arguments::MatmulArguments};
use crate::{
    DataType,
...
/// MLP Fused GEMV configuration
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
struct MlpFusedGemvConfiguration {
    threadgroup_rows: u32,
    threadgroup_cols: u32,
    threads_per_simdgroup_row: u32,
    threads_per_simdgroup_col: u32,
...
impl MlpFusedGemvConfiguration {
    fn output_elements_per_threadgroup(&self) -> u32 {
        self.threadgroup_rows
            * self.threads_per_simdgroup_row
            * self.elements_per_thread_row
...
fn mlp_fused_gemv_kernel_name(
    data_type: DataType,
    config: &MlpFusedGemvConfiguration,
) -> Result<String, MTLError> {
    let dtype_name = match data_type {
        DataType::F16 => "float16",
        DataType::BF16 => "bfloat16",
...
fn select_mlp_fused_gemv_configuration(
    hidden_dim: i32,
    activation: MlpActivationType,
) -> MlpFusedGemvConfiguration {
    // Select configuration based on hidden dimension
...
/// Arguments for MLP fused GEMV
#[derive(Debug)]
pub struct MlpFusedGemvArguments<'a> {
    /// Weight matrix [2*hidden_dim, input_dim] (up and gate concatenated)
    ...
    pub hidden_dim: i32,
    /// Weight matrix leading dimension
    pub weights_ld: i32,
    /// Batch count
    pub batch_count: i32,
    /// Activation type
...
/// MLP Fused GEMV Kernel
/// Computes paired up/gate projections with fused activation: out = up * activation(gate)
pub struct MlpFusedGemvKernel {
    data_type: DataType,
    pipelines: HashMap<MlpFusedGemvConfiguration, ComputePipelineState>,
}

impl MlpFusedGemvKernel {
    ...
    fn get_pipeline(
        ...
        if !self.pipelines.contains_key(&config) {
            let kernel_name =
                mlp_fused_gemv_kernel_name(self.data_type, &config)?;
    ...
    pub fn encode(
        ...
        context: &MTLContext,
        encoder: &ComputeCommandEncoderRef,
        args: &MlpFusedGemvArguments,
    ) -> Result<(), MTLError> {
        let config = select_mlp_fused_gemv_configuration(
            args.hidden_dim,
            args.activation,
        );
        ...
        let matrix_batch_stride = 0i64; // Weights not batched
        encoder.set_bytes(
            11,
