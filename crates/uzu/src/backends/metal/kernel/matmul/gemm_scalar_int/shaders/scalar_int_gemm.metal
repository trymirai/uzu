...
// i8 * i16 -> i16 (accumulate in int)
KERNEL(ScalarIntGemmI8I16I16)(
    const device int8_t* a,
    const device short* b,
    device short* d,
    const constant uzu::matmul::GEMMParams* params,
    const constant uint& group_count_x,
    const constant uint& group_count_y,
    const constant uint& group_count_z,
    threadgroup int8_t a_shared[TILE_M * TILE_K],
    threadgroup short b_shared[TILE_N * TILE_K],
    const uint group_x GROUPS(group_count_x),
    const uint group_y GROUPS(group_count_y),
    const uint group_z GROUPS(group_count_z),
    const uint thread_x THREADS(32),
  ...
  const uint thread_idx = thread_y * 32 + thread_x;
  scalar_int_gemm_impl<int8_t, short, short, int>(
      a, b, d, params,
      a_shared, b_shared,
      uint3(group_x, group_y, group_z),
      uint3(thread_x, thread_y, 0),
...
// i16 * i16 -> f32 (accumulate in float)
KERNEL(ScalarIntGemmI16I16F32)(
    const device short* a,
    const device short* b,
    device float* d,
    const constant uzu::matmul::GEMMParams* params,
    const constant uint& group_count_x,
    const constant uint& group_count_y,
    const constant uint& group_count_z,
    threadgroup short a_shared[TILE_M * TILE_K],
    threadgroup short b_shared[TILE_N * TILE_K],
    const uint group_x GROUPS(group_count_x),
    const uint group_y GROUPS(group_count_y),
    const uint group_z GROUPS(group_count_z),
  ...
  const uint thread_idx = thread_y * 32 + thread_x;
  scalar_int_gemm_impl<short, short, float, float>(
      a, b, d, params,
      a_shared, b_shared,
      uint3(group_x, group_y, group_z),
      uint3(thread_x, thread_y, 0),
