...
impl Specialization {
    ...
    pub fn precompile_configs(data_type: DataType) -> &'static [Self] {
        match data_type {
            DataType::BF16 => &[
                Self {
                    threadgroup_rows: 4, threadgroup_cols: 1,
                    threads_per_simdgroup_row: 1, threads_per_simdgroup_col: 32,
                    elements_per_thread_row: 4, elements_per_thread_col: 4,
                ...
                Self {
                    threadgroup_rows: 8, threadgroup_cols: 1,
                    threads_per_simdgroup_row: 1, threads_per_simdgroup_col: 32,
                    elements_per_thread_row: 4, elements_per_thread_col: 4,
                    apply_output_scale_and_accumulate: false,
                },
    ...
    pub fn select(
        ...
        let thread_output_rows = if output_dimension < 4 { 1 } else { 4 };
