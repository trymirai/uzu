...
struct ThreadgroupLoader {
  METAL_CONST short ROW_ITERATIONS = (BLOCK_ROWS + THREAD_ROWS - 1) / THREAD_ROWS;
  ...
  METAL_FUNC ThreadgroupLoader(
      ...
      : source_leading_dimension(source_leading_dim),
        tile_stride(REDUCTION_DIMENSION ? BLOCK_COLS : BLOCK_ROWS * source_leading_dim),
        ...
        destination(destination_ptr + block_row_index * DESTINATION_LEADING_DIMENSION + block_col_index),
        source(source_ptr + block_row_index * source_leading_dim + block_col_index) {}
  ...
  METAL_FUNC void load_unsafe() const {
    METAL_PRAGMA_UNROLL
    for (short i = 0; i < BLOCK_ROWS; i += THREAD_ROWS) {
      *((threadgroup ReadVector*)(&destination[i * DESTINATION_LEADING_DIMENSION])) =
          *((const device ReadVector*)(&source[i * source_leading_dimension]));
    }
  }

  METAL_FUNC void load_safe(short2 source_tile_dimensions) const {
    source_tile_dimensions = source_tile_dimensions - short2(block_col_index, block_row_index);
