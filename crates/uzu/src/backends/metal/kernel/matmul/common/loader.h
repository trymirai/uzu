...
struct BlockLoader {
  ...
  METAL_FUNC BlockLoader(
      ...
      : source_leading_dimension(source_leading_dimension_),
        tile_stride(reduction_dim ? BLOCK_COLS : BLOCK_ROWS * source_leading_dimension),
        thread_idx(simd_group_id * 32 + simd_lane_id),
        block_row(thread_idx / THREAD_COLS),
        block_col(VECTOR_SIZE * (thread_idx % THREAD_COLS)),
        destination(destination_ + block_row * destination_leading_dimension + block_col),
  ...
  METAL_FUNC void apply_inplace_op(thread const UnaryOp& op) const {
    ...
    for (short i = 0; i < BLOCK_ROWS; i += THREAD_ROWS) {
      ...
      for (short j = 0; j < VECTOR_SIZE; j++) {
        destination[i * destination_leading_dimension + j] = op.apply(destination[i * destination_leading_dimension + j]);
      }
    }
  }
  ...
  METAL_FUNC void load_checked(short2 source_tile_dimensions) const {
    ...
    for (short i = 0; i < BLOCK_ROWS; i += THREAD_ROWS) {
      ...
      PRAGMA_UNROLL
      for (short j = 0; j < VECTOR_SIZE; j++) {
        valid_indices[j] = (i < source_tile_dimensions.y) && (j < source_tile_dimensions.x);
      }

      // Read valid indices into temp_values
      PRAGMA_UNROLL
      for (short j = 0; j < VECTOR_SIZE; j++) {
        temp_values[j] = source[(valid_indices[j] ? i * source_leading_dimension + j : 0)];
      }
