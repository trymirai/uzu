...
METAL_FUNC void gemm_mpp_staged(
  ...
  device T* output_ptr =
      output_matrix + block_row_start_long * params->leading_dim_d +
      block_col_start_long + tile_row_offset * params->leading_dim_d +
      tile_col_offset;

  const int simdgroup_limit_m_int =
      align_m ? int(SIMDGROUP_M)
              : min(int(SIMDGROUP_M),
  ...
  const bool is_unaligned_m =
      align_m ? false : (simdgroup_limit_m != SIMDGROUP_M);
  const bool is_unaligned_n =
      align_n ? false : (simdgroup_limit_n != SIMDGROUP_N);
  ...
  for (short tile_m = 0; tile_m < TILES_M; tile_m++) {
    ...
    for (short tile_n = 0; tile_n < TILES_N; tile_n++) {
      ...
      device T* output_subtile_ptr =
          output_ptr + row_offset * params->leading_dim_d + col_offset;
      ...
      for (short i = 0; i < accumulator_capacity; i++) {
        if (subtile_aligned_m && subtile_aligned_n) {
          if (output_valid[i]) {
            output_subtile_ptr
                [output_row[i] * params->leading_dim_d + output_col[i]] =
