...
#define instantiate_mlp_fused_gemm(                                            \
  ...
  template [[host_name(                                                        \
      "steel_gemm_mlp_fused_" #tname "_" #itype "_" #otype "_bm" #bm "_bn" #bn \
      "_bk" #bk "_wm" #wm "_wn" #wn "_" #aname "_MN_" #mn "_K_" #k             \
  )]] [[kernel]] void                                                          \
...
steel_gemm_mlp_fused(
  ...
  using kernel_t = steel::GEMMMlpFusedKernel<
      T,
      U,
      BM,
      BN,
...
// transpose_a = false, transpose_b = true (weights are [K, 2*H], accessed as
// transposed)

// float16 configurations
instantiate_mlp_fused_gemm(
    t,
    false,
    true,
    half,
