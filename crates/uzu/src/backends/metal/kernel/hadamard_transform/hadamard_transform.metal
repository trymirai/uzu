...
PUBLIC KERNEL(HadamardTransformMul)(
  ...
  uint element_index = batch_index * channel_count +
                       block_within_batch * METAL_SIMD_SIZE + lane_index;
  ...
  for (uint stride = 1; stride < METAL_SIMD_SIZE; stride <<= 1) {
    ...
    value = (lane_index & stride) ? (partner_value - value)
                                  : (partner_value + value);
  }

  // Orthogonal normalization: matches Python hadamard_transform /
  // sqrt(block_size)
  constexpr float normalization_factor =
      1.0f / 5.656854249f; // 1/sqrt(METAL_SIMD_SIZE)
