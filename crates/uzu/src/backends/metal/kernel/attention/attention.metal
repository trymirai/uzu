...
void attention_single_pass_impl(
  ...
  const int q_offset = query_transposed ? tpg.x * q_seq_idx + head_idx
...
void attention_2pass_1_impl(
  ...
  keys += kv_head_idx * k_head_stride +
          (block_idx * sequence_block_size + simd_gid) * k_seq_stride +
          simd_lid * qk_elements_per_thread;
  values += kv_head_idx * v_head_stride +
            (block_idx * sequence_block_size + simd_gid) * v_seq_stride +
            simd_lid * value_elements_per_thread;
  ...
  if (bool_mask) {
    bmask += head_idx * mask_head_stride +
             (block_idx * sequence_block_size + simd_gid) * mask_kv_seq_stride +
             q_seq_idx * mask_q_seq_stride;
  }
  if (float_mask) {
  ...
  // For each key
  for (int i = block_idx * sequence_block_size + simd_gid; i < sequence_length;
       i += total_blocks_count * sequence_block_size) {
    bool use_key = true;
    if (do_causal) {
      use_key = i <= (sequence_length - int(tpg.y) + int(q_seq_idx));
    } else if (bool_mask) {
  ...
  threadgroup_barrier(mem_flags::mem_threadgroup);
  max_score =
      (simd_lid < sequence_block_size) ? shared_max_scores[simd_lid] : -1e9;
  U new_max = simd_max(max_score);
  U factor = fast::exp(max_score - new_max);
  sum_exp_score =
      (simd_lid < sequence_block_size) ? shared_sum_exp_scores[simd_lid] : 0;
  ...
  // Now we need to aggregate all the outputs
  for (int i = 0; i < value_elements_per_thread; i++) {
    shared_outputs[simd_lid * sequence_block_size + simd_gid] =
...
#define GENERATE_SINGLE_PASS_KERNELS(head_dim_value)                           \
  [[max_total_threads_per_threadgroup(                                         \
      1024                                                                     \
  )]] kernel void attention_single_pass_half_##head_dim_value                  \
  outerArguments(half) {                                                       \
    constexpr int sequence_block_size = 32;                                    \
    constexpr int head_block_size = 32;                                        \
  ...
  [[max_total_threads_per_threadgroup(                                         \
      1024                                                                     \
  )]] kernel void attention_single_pass_bfloat_##head_dim_value                \
  ...
  [[max_total_threads_per_threadgroup(                                         \
      1024                                                                     \
  )]] kernel void attention_single_pass_float_##head_dim_value                 \
...
#define GENERATE_2PASS_1_KERNELS(head_dim_value)                               \
  [[max_total_threads_per_threadgroup(                                         \
      256                                                                      \
  )]] kernel void attention_2pass_1_half_##head_dim_value                      \
  outerArguments(half) {                                                       \
    constexpr int sequence_block_size = 8;                                     \
    constexpr int head_block_size = 32;                                        \
    threadgroup float shared_max_scores[sequence_block_size];                  \
  ...
  [[max_total_threads_per_threadgroup(                                         \
      256                                                                      \
  )]] kernel void attention_2pass_1_bfloat_##head_dim_value                    \
  outerArguments(bfloat) {                                                     \
    constexpr int sequence_block_size = 8;                                     \
    constexpr int head_block_size = 32;                                        \
    threadgroup float shared_max_scores[sequence_block_size];                  \
  ...
  [[max_total_threads_per_threadgroup(                                         \
      256                                                                      \
  )]] kernel void attention_2pass_1_float_##head_dim_value                     \
  outerArguments(float) {                                                      \
    constexpr int sequence_block_size = 8;                                     \
    constexpr int head_block_size = 32;                                        \
    threadgroup float shared_max_scores[sequence_block_size];                  \
...
#define GENERATE_2PASS_2_KERNELS(head_dim_value)                               \
  [[max_total_threads_per_threadgroup(                                         \
      1024                                                                     \
  )]] kernel void attention_2pass_2_half_##head_dim_value                      \
  outerArguments(half) {                                                       \
    constexpr int sequence_block_size = 32;                                    \
    constexpr int head_block_size = 32;                                        \
    threadgroup float shared_outputs[sequence_block_size * head_block_size];   \
  ...
  [[max_total_threads_per_threadgroup(                                         \
      1024                                                                     \
  )]] kernel void attention_2pass_2_float_##head_dim_value                     \
  outerArguments(float) {                                                      \
    constexpr int sequence_block_size = 32;                                    \
    constexpr int head_block_size = 32;                                        \
    threadgroup float shared_outputs[sequence_block_size * head_block_size];   \
...
void update_kv_cache(
    ...
    const device T*
        qkv, // [suffix_length, (num_heads + 2*num_groups) * head_dim]
  ...
  if (groupIndex >= num_groups || tokenIndex >= suffix_length ||
      dimIndex >= head_dim) {
    return;
  }
