...
VARIANTS(BITS, 4, 8)
KERNEL(QuantizedMatmulGemmTransposedV2)(
  ...
  if (use_mlx_quant) {
    if (aligned_n) {
      quantized_gemm_transposed_implementation<T, GROUP_SIZE, BITS, true, 32, 32, 32, true>(
          w, scales, zero_points, biases, x, y,
          input_shared, weight_shared, k, n, m,
          tid, lid, simd_gid, simd_lid);
    } else {
      quantized_gemm_transposed_implementation<T, GROUP_SIZE, BITS, false, 32, 32, 32, true>(
          w, scales, zero_points, biases, x, y,
          input_shared, weight_shared, k, n, m,
          tid, lid, simd_gid, simd_lid);
    }
    ...
    if (aligned_n) {
      quantized_gemm_transposed_implementation<T, GROUP_SIZE, BITS, true, 32, 32, 32, false>(
          w, scales, zero_points, biases, x, y,
          input_shared, weight_shared, k, n, m,
          tid, lid, simd_gid, simd_lid);
    } else {
      quantized_gemm_transposed_implementation<T, GROUP_SIZE, BITS, false, 32, 32, 32, false>(
          w, scales, zero_points, biases, x, y,
          input_shared, weight_shared, k, n, m,
          tid, lid, simd_gid, simd_lid);
    }
