...
// Fused TensorAddSwap + RMSNorm: computes residual_sum = main + shortcut,
// writes residual_sum to shortcut, normalizes residual_sum, writes to main.
// Eliminates one separate TensorAddSwap dispatch per layer.
...
PUBLIC KERNEL(ResidualAddRMSNorm)(
  ...
  // Pass 1: compute residual sum, write to shortcut, accumulate sum-of-squares
  for (uint base_i = thread_in_row * GRAIN_SIZE; base_i < element_count;
    ...
    for (uint j = 0; j < GRAIN_SIZE; ++j) {
      ...
      if (i < element_count) {
        ...
        main_data[i] = static_cast<DataT>(residual);
      } else {
        vals[j] = 0.0f;
      }
    }
  ...
  // Pass 2: normalize the residual sum (re-read from main), write to main
  for (uint base_i = thread_in_row * GRAIN_SIZE; base_i < element_count;
       base_i += BLOCK_SIZE * GRAIN_SIZE) {
    AccumT vals[GRAIN_SIZE];
