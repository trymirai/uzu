...
void batched_topp(
    ...
    for (uint iter = 0; iter < MAX_ITERS; iter++) {
        ...
        for (uint i = thread_idx; i < vocab_size; i += BLOCK_SIZE) {
            ...
            local_min_above_threshold = fmin(local_min_above_threshold, select(INFINITY, logit_mass, logit_value >= threshold));
        }
        ...
        float min_above_threshold = threadgroup_cooperative_reduce_min<BLOCK_SIZE>(local_min_above_threshold, shared_reduce_buffer, thread_idx);
