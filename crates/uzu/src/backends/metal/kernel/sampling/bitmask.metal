...
void batched_bitmask(
    ...
    for (uint i = 0; i < GRAIN_SIZE; i++) {
        ...
        if (global_idx < batch_end) {
            bool mask = (bitmask_data[global_idx / BITS_IN_U32] >> (global_idx % BITS_IN_U32)) & 0b1;
            processed_logits[global_idx] = select(T(-INFINITY), logits_data[global_idx], mask);
        }
    }
}
...
#define innerArguments (logits_data, bitmask_data, processed_logits, vocab_size, threadgroup_idx.x, threadgroup_idx.y, thread_idx.x)
...
generateBitmaskKernel(functionName, float, outerArguments(float), innerArguments); \
generateBitmaskKernel(functionName, bfloat, outerArguments(bfloat), innerArguments); \
generateBitmaskKernel(functionName, half, outerArguments(half), innerArguments);
