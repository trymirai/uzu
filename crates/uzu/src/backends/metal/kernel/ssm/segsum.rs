...
impl Cumsum1DKernel {
    ...
    pub fn encode(
        ...
        compute_encoder.set_bytes(
            ...
            &args.length as *const usize as *const _,
        );
        compute_encoder.set_bytes(
            3,
            size_of::<usize>() as u64,
            &args.outer_size as *const usize as *const _,
        );

        let threads_per_grid = MTLSize {
            width: args.outer_size as u64,
            height: 1,
            depth: 1,
        };
...
impl SegsumFromCumsumKernel {
    pub fn new(
        ...
        let fn_name =
            format!("segsum_from_cumsum_kernel_{}", fn_suffix(data_type));
        let pipeline = context
            .compute_pipeline_state(&fn_name, None)
            .map_err(SSMKernelError::MetalError)?;
        Ok(Self {
