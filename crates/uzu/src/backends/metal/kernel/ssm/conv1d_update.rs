...
impl Conv1dUpdateKernel {
    ...
    pub fn encode(
        ...
        compute_encoder.set_bytes(6, size_of::<i32>() as u64, &args.kernel_size as *const i32 as *const _);
        compute_encoder.set_bytes(7, (2 * size_of::<usize>()) as u64, args.x_strides.as_ptr() as *const _);
        compute_encoder.set_bytes(8, (3 * size_of::<usize>()) as u64, args.state_strides.as_ptr() as *const _);

        let threads_per_threadgroup = MTLSize { width: 32, height: 1, depth: 1 };
        let total_threads = MTLSize { width: args.batch_size as u64, height: args.channels as u64, depth: 1 };
