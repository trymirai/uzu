...
pub struct SSDUpdateNoZKernel {
    pipeline: MTLComputePipelineState,
}

pub struct SSDUpdateNoZArguments<'a> {
...
impl SSDUpdateNoZKernel {
    pub fn new(context: &MTLContext, data_type: KernelDataType) -> Result<Self, SSMKernelError> {
        let fn_name = format!("ssd_update_no_z_kernel_{}", fn_suffix(data_type));
    ...
    pub fn encode(
        ...
        compute_encoder.set_buffer(8, Some(args.next_state), 0);

        compute_encoder.set_bytes(9, size_of::<i32>() as u64, &args.group_size as *const i32 as *const _);
        compute_encoder.set_bytes(10, size_of::<i32>() as u64, &args.state_size as *const i32 as *const _);
        compute_encoder.set_bytes(11, (3 * size_of::<usize>()) as u64, args.x_strides.as_ptr() as *const _);
        compute_encoder.set_bytes(12, (2 * size_of::<usize>()) as u64, args.dt_strides.as_ptr() as *const _);
        compute_encoder.set_bytes(13, (3 * size_of::<usize>()) as u64, args.cb_strides.as_ptr() as *const _);
        compute_encoder.set_bytes(14, (4 * size_of::<usize>()) as u64, args.state_strides.as_ptr() as *const _);
