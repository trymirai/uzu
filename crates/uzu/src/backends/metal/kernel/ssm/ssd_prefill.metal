...
constant uint SSM_PREFILL_MAX_STATE = 256;

template <typename T>
VARIANTS(T, float, half, bfloat)
KERNEL(SSDPrefillSingle64)(
    ...
    constant const uint state_strides[3],
    constant const uint& num_heads,
    constant const uint& head_dim,
    const uint tg_pos_x AXIS(num_heads * head_dim * 32, 32),
    const uint tg_pos_y AXIS(1, 1)
) {
  const uint lane_idx = tg_pos_x;
  const uint pair_idx = tg_pos_x;
  const uint simd_width = 32u;
  const uint state_dim = state_size;
...
KERNEL(SSDPrefillSingle)(
  ...
  const uint max_chunks = SSM_PREFILL_MAX_STATE / simd_width;
  const uint chunk_count = (state_dim + simd_width - 1) / simd_width;
...
#pragma unroll
  for (uint chunk = 0; chunk < chunk_count; ++chunk) {
    const uint idx = chunk * int(simd_width) + int(lane_idx);
    const uint state_idx = state_base + size_t(idx) * state_inner_stride;
...
#pragma unroll
    for (uint chunk = 0; chunk < chunk_count; ++chunk) {
      const uint idx = chunk * simd_width + lane_idx;
      const uint cb_idx = cb_group_base + idx * cb_state_stride + token * cb_token_stride;
...
#pragma unroll
  for (uint chunk = 0; chunk < chunk_count; ++chunk) {
    const int idx = chunk * simd_width + lane_idx;
    const size_t state_idx = state_base + idx * state_inner_stride;
...
KERNEL(SSDPrefillSequential)(
  ...
  const uint safe_group = max(group_size, 1u);
  const uint group_idx = h_idx / safe_group;
  device T* state_row = state + h_idx * state_strides[0] + size_t(dh_idx) * state_strides[1];

  for (size_t token = 0; token < suffix_len; ++token) {
    const size_t x_idx = token * x_strides[0] + h_idx * x_strides[1] + dh_idx * x_strides[2];
    const size_t dt_idx = token * dt_strides[0] + h_idx * dt_strides[1];
    const size_t cb_base = token * cb_strides[0] + group_idx * cb_strides[1];
    ...
    T acc = T(0);
    int s = 0;
    const uint vec_bound = (state_size / 4) * 4;
    for (; s < vec_bound; s += 4) {
