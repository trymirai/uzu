...
fn repository(paths: Option<Vec<String>>) -> Repository {
    Repository {
...
async fn hugging_face_files() -> Result<(), Box<dyn std::error::Error>> {
    ...
    mount_hugging_face(&server, 2).await;
    let hugging_face = HuggingFace::builder().endpoint(server.uri()).build()?;

    let files = hugging_face.files(&repository(None)).await?;
    ...
    let filtered = hugging_face.files(&repository(Some(vec!["config.json".to_string()]))).await?;
...
async fn hugging_face_rejects_bad_metadata() -> Result<(), Box<dyn std::error::Error>> {
    for body in [
        json!({ "sha": "0000000000000000000000000000000000000000", "siblings": siblings() }),
        ...
        let hugging_face = HuggingFace::builder().endpoint(server.uri()).build()?;
        assert!(hugging_face.files(&repository(None)).await.is_err());
    }
    Ok(())
}
...
async fn mirai_registry_resolves_pinned_models_once() -> Result<(), Box<dyn std::error::Error>> {
    ...
    mount_hugging_face(&hugging_face, 1).await;
    let mirai = MockServer::start().await;
    let pinned = |identifier: &str| {
        json!({
            "type": "mirai",
            "toolchain_version": "1",
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            ...
            "models": [
                listed_model("pinned", pinned("trymirai/model")),
                ...
                listed_model("unresolvable", pinned("trymirai/missing")),
            ]
        })))
        .mount(&mirai)
        .await;
    ...
    for _ in 0..2 {
        ...
        assert_eq!(models.iter().map(|model| model.identifier.as_str()).collect::<Vec<_>>(), ["pinned", "unpinned"]);
        let ModelAccessibility::OnDevice {
            ...
            panic!("pinned model must stay on device");
        };
        assert_eq!(files.len(), 2);
        assert_eq!(files[1].hashes[0].method, HashMethod::Sha256);
        assert!(files.iter().all(|file| file.url.starts_with(&hugging_face.uri())));
    }
    Ok(())
}
