...
The opt-in `hardware-control` feature exposes macOS-only fan, power-mode and GPU
budget APIs. Private symbols resolve through `kanka`, and private names remain
obfuscated.

The CLI's Fast mode sets each fan to its reported maximum and requests macOS High
Power Mode. It captures the prior settings and restores them on Auto, normal exit,
connection loss, SIGINT, SIGTERM and SIGHUP. SIGKILL and power loss cannot run
restoration.

`GpuPowerControl::maximum_power_milliwatts` is diagnostic only. It reports a
calibrated budget, not actual draw, an enforced ceiling, fixed clocks or guaranteed
performance.
