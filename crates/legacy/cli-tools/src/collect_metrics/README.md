...
`uzu-tools` (the CLI's `bin_name`) is used as shorthand in the examples below — substitute the `cargo run --release --features collect-metrics -p cli-tools -- collect-metrics` prefix, or the `cargo collect-metrics` alias from `.cargo/config.toml` (debug profile). The repo's `cargo tools` alias does **not** work here: it doesn't enable the `collect-metrics` feature.

## Modes

### Registry (default)

Uses the live Mirai registry. `config.json` and `tokenizer.json` are downloaded through `Storage`; safetensors headers are range-fetched and cached beside them.
...
Registry mode writes `config.json` and `tokenizer.json` through `Storage` and saves the HTTP range-fetched safetensors header as `model.header.safetensors`.
...
- `tokenizer.json` must sit beside `config.json`; the engine loads it even though the sweep feeds raw token ids.
...
2. Place a valid Uzu `config.json`, its `tokenizer.json`, and a safetensors header (or full `model.safetensors`) in each model directory.
