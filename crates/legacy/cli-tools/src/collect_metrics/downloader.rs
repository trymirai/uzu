...
use super::{
    artifacts::{CACHE_NAME, CONFIG_FILE, HEADER_FILE, TOKENIZER_FILE},
...
/// Fetches a model's `config.json`, `tokenizer.json`, and safetensors header, but never its weights.
...
impl Downloader {
    pub async fn new(storage_base: Option<PathBuf>) -> Result<Self> {
        ...
        let storage_config = StorageConfig::new(storage_device, storage_base, CACHE_NAME.to_string())
            .with_download_contents(DownloadContents::CONFIG | DownloadContents::TOKENIZER);
        let storage =
    ...
    pub async fn fetch(
        ...
        for name in [CONFIG_FILE, TOKENIZER_FILE] {
            if !model_dir.join(name).is_file() {
                bail!("{name} missing after download");
            }
        }
