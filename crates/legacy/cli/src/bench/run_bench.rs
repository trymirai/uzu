...
pub async fn run_bench(
    ...
    let speculator_stats = results.iter().filter_map(|result| result.speculator_stats.as_ref()).collect::<Vec<_>>();
    let tokens_per_pass_list = speculator_stats.iter().map(|stats| stats.tokens_per_forward_pass).collect::<Vec<f64>>();
    let rows_per_pass_list =
        speculator_stats.iter().map(|stats| stats.proposed_tokens_per_forward_pass).collect::<Vec<f64>>();
    ...
    let tokens_per_pass_metric = calculate_metric(&tokens_per_pass_list);
    let rows_per_pass_metric = calculate_metric(&rows_per_pass_list);
    ...
    table
        ...
        .add_row(vec!["Accepted tokens per pass", tokens_per_pass_metric.as_str()])
        .add_row(vec!["Rows per pass", rows_per_pass_metric.as_str()]);
