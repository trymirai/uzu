...
use crate::{
    ...
    data_type::DataType,
    tests::helpers::{buffer_prefix_to_vec, buffer_to_vec, create_buffer, create_buffer_with_data},
};
...
fn run_delta_net_update<B: Backend>(
    ...
    let in_proj_array = create_buffer_with_data::<B, f32>(&context, in_proj);
    ...
    let mut state_buffer = create_buffer_with_data::<B, f32>(&context, state);
    let mut out = create_buffer::<B, f32>(&context, value_dim as usize);
    ...
    let out = buffer_to_vec::<B, f32>(&out);
    let new_state = buffer_to_vec::<B, f32>(&state_buffer);
    (out, new_state)
}
...
fn run_prefill_with_norm_gate_typed<T: ArrayElement>(
    ...
    let context = <Metal as Backend>::Context::new().expect("context");
    let in_proj_array = create_buffer_with_data::<Metal, T>(&context, in_proj);
    ...
    let mut state_array = create_buffer_with_data::<Metal, f32>(&context, state);
    let mut out_array = create_buffer::<Metal, T>(&context, suffix_len * value_dim);
