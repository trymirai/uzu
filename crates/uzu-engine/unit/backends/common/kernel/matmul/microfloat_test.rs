...
fn check_dense_mxfp4<T: ArrayElement>(
    ...
    let _completed = command_buffer.wait_until_completed().expect("execute MXFP4 matmul");
...
fn cpu_rejects_invalid_mxfp4_operands_before_dispatch() {
    ...
    for (metadata, code_bytes, scale_bytes, outer_bytes, invalid_metadata) in [
        ...
        let CpuError::KernelDispatchFailed(error) = error else {
            panic!("unexpected CPU error: {error}");
        };
        let error = error.downcast::<MatmulError<Cpu>>().expect("preserve matmul error");
