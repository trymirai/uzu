...
use metal::{
    ...
    MTLGPUFamily, MTLLibrary, MTLResidencySet, MTLResidencySetDescriptor, MTLResourceOptions, MTLSparsePageSize,
};
...
pub struct MetalContext {
    ...
    pub(super) allocator: Arc<Allocator<Metal>>,
...
impl Context for MetalContext {
    ...
    fn create_buffer(
        &self,
        size: usize,
    ) -> Result<MetalDenseBuffer, MetalError> {
        let buffer = self
            .device
            .new_buffer(size, MTLResourceOptions::STORAGE_MODE_SHARED)
            .ok_or(MetalError::CannotCreateBuffer)?;

        let buffer = MetalDenseBuffer::new(buffer, self.residency_set.clone());
    ...
    fn create_allocation_pool(&self) -> Arc<AllocationPool<Metal>> {
    ...
    fn create_command_buffer(
        &self,
        name: Option<&str>,
        allocation_pool: Option<Arc<AllocationPool<Metal>>>,
        ...
        Ok(MetalCommandBufferEncoding::new(command_allocator, command_buffer, context, allocation_pool))
    }
