...
impl<B: Backend> MoeExpertsTwoPassPrefillBlock<B> {
    ...
    pub fn encode(
        ...
        let mut tile_counts =
            command_buffer.allocate_scratch(size_for_shape(&[args.num_routed_experts], DataType::U32))?;
        self.counts.encode(args.expert_offsets, &mut tile_counts, args.num_routed_experts, command_buffer);

        let mut tile_offsets =
            command_buffer.allocate_scratch(size_for_shape(&[args.num_routed_experts + 1], DataType::U32))?;
        let mut total_tiles = command_buffer.allocate_scratch(size_for_shape(&[8], DataType::U32))?;
        self.scan.encode(&tile_counts, &mut tile_offsets, &mut total_tiles, args.num_routed_experts, command_buffer);

        let mut tile_map = command_buffer.allocate_scratch(size_for_shape(&[args.total_rows, 3], DataType::U32))?;
        self.build.encode(
            args.expert_offsets,
            &tile_offsets,
            &tile_counts,
        ...
        let mut pass_a_dispatch_args = command_buffer.allocate_scratch(size_for_shape(&[3], DataType::U32))?;
        self.dispatch.encode(&total_tiles, &mut pass_a_dispatch_args, n_tiles_ff, command_buffer);

        let mut hidden =
            command_buffer.allocate_scratch(size_for_shape(&[args.total_rows, args.d_ff], DataType::F32))?;
        ...
        let mut pass_b_dispatch_args = command_buffer.allocate_scratch(size_for_shape(&[3], DataType::U32))?;
        self.dispatch.encode(&total_tiles, &mut pass_b_dispatch_args, n_tiles_model, command_buffer);

        let mut output =
            command_buffer.allocate_scratch(size_for_shape(&[args.total_rows, args.d_model], self.data_type))?;
