...
use crate::{
    ...
    backends::common::{
        ...
        CommandBufferEncodingExt, gpu_types::trie::TrieNode,
    },
...
impl<B: Backend> DFlash<B> {
    ...
    pub fn encode_accept(
        ...
        let mut packed_target_features = command_buffer
            .allocate_scratch(size_for_shape(&[num_tokens, self.target_feature_input_dim], self.data_type))?;
        for (layer_index, features) in target_features.iter().enumerate() {
            for (token_index, &accepted_index) in accepted_indices.iter().enumerate() {
                ...
                command_buffer.encode_copy(
                    features.as_buffer_range_ref().subrange(source_start..source_start + layer_feature_bytes),
                    packed_target_features
                        .as_buffer_range_mut()
                        .subrange(destination_start..destination_start + layer_feature_bytes),
                );
            }
        }
        ...
        let normalized_features =
            self.projected_feature_norm.encode(&projected_features, 0, num_tokens, None, command_buffer)?;
        ...
        for (layer_index, key_value) in layer_key_values.iter_mut().enumerate() {
            for token_index in 0..num_tokens as usize {
                command_buffer.encode_copy(
                    projected_kv
                        .as_buffer_range_ref()
                        .subrange(kv_chunk(token_index * self.layers.len() + layer_index)),
                    key_value.as_buffer_range_mut().subrange(kv_chunk(token_index)),
                );
            }
        }
            ...
            self.layers.iter().zip(state.layer_states.iter_mut()).zip(layer_key_values)
        {
            ...
            attention.append_projected_kv(key_value, &rope, num_tokens, attention_state, command_buffer)?;
        }
    ...
    pub fn encode_draft(
        ...
        let mut lookahead_hidden =
            command_buffer.allocate_scratch(lookahead_rows.len()).map_err(DFlashEncodeError::Backend)?;
        command_buffer.encode_copy(
            draft_hidden.as_buffer_range_ref().subrange(lookahead_rows),
            lookahead_hidden.as_buffer_range_mut(),
        );
        let logits =
            target_embedding.encode_readout_raw(batch_size - 1, &lookahead_hidden, DataType::F32, command_buffer)?;
