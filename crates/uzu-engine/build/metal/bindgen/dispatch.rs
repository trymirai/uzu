...
fn build_axis_dispatch(
    ...
    let dispatch_code = quote! {
        command_buffer.compute_encoder.dispatch_threads_threads_per_threadgroup(
...
fn build_indirect_dispatch(
    ...
    let dispatch_code = quote! {
        command_buffer.compute_encoder.dispatch_threadgroups_with_indirect_buffer_threads_per_threadgroup(
            __dsl_indirect_dispatch_buffer.0.gpu_address() + __dsl_indirect_dispatch_buffer.1.start as u64,
            MTLSize::new(#((#threads) as usize, )*),
        );
    };
...
fn build_direct_dispatch(
    ...
    let dispatch_code = quote! {
        command_buffer.compute_encoder.dispatch_threadgroups_threads_per_threadgroup(
