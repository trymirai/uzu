...
pub fn generate_swift_extensions(environment: &Environment) -> Result<(), Error> {
    let (factories, callback_factories, stream_bridges) = collect_all(&environment.crates_path())?;
    let swift_source = render_extensions_file(&factories, &callback_factories, &stream_bridges);

    let destination =
        environment.bindings().swift_path().join("Sources").join("Uzu").join("Generated").join("Extensions.swift");

    if let Some(parent) = destination.parent() {
        fs::create_dir_all(parent).map_err(|_| Error::UnableToPrepareBindingsSwift)?;
    }
    fs::write(&destination, swift_source).map_err(|_| Error::UnableToPrepareBindingsSwift)?;
...
fn collect_all(crates_path: &Path) -> Result<(Vec<Factory>, Vec<CallbackFactory>, Vec<StreamBridge>), Error> {
...
fn render_extensions_file(
    ...
    output.push_str("// Auto-generated by cli-release — do not edit.\n\n");
