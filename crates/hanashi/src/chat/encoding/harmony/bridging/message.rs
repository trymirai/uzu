...
use openai_harmony::chat::{
    ...
    Message as ExternalMessage, ReasoningEffort as ExternalReasoningEffort, Role as ExternalRole,
    SystemContent as ExternalSystemContent, TextContent, ToolNamespaceConfig as ExternalToolNamespaceConfig,
};

use crate::chat::{
    encoding::harmony::bridging::Error,
    types::{ContentBlock, Message, ReasoningEffort, Role, ToolCall, ToolNamespace, Value},
};
                ...
                for block in &message.content {
                    match block {
                        ...
                        ContentBlock::ReasoningEffort {
                            ...
                            system_filled = true;
                            system_content.reasoning_effort = Some(ExternalReasoningEffort::try_from(value.clone())?);
                        },
                        ...
                        ContentBlock::Tools {
                            ...
                            for namespace in namespaces {
                                developer_content = developer_content
                                    .with_tools(ExternalToolNamespaceConfig::try_from(namespace.clone())?);
                            }
                        },
                        other => {
                            return Err(Error::UnsupportedContentBlock {
                        ...
                        ExternalContent::SystemContent(system_content) => {
                            ...
                            if let Some(effort) = &system_content.reasoning_effort {
                                content.push(ContentBlock::ReasoningEffort {
                                    value: ReasoningEffort::from(*effort),
                                });
                            }
                        ...
                        ExternalContent::DeveloperContent(developer_content) => {
                            ...
                            if let Some(tools) = &developer_content.tools {
                                let namespaces: Vec<ToolNamespace> =
                                    tools.values().cloned().map(ToolNamespace::from).collect();
