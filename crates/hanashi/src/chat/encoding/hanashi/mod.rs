...
pub use error::Error;
use token_stream_parser::{Parser as _, token_stream::TokenStreamParser, types::Token as ParserToken};
...
use crate::{
    ...
    chat::{
        encoding::{
            ...
            hanashi::{messages::rendered::FieldConfig, renderer::Renderer},
        },
        types::{ContentBlock, Message, Token, TokenId},
    },
};
...
impl EncodingTrait for Encoding {
    ...
    fn encode(
        ...
        for token_id in text_encoding.get_ids() {
            ...
            self.parser.push(&ParserToken::from(token.clone()))?;
