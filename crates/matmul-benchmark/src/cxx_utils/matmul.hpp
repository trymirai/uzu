...
// Input: A[M,K] row-major, B[K,N] row-major (but we'll transpose B for consistency with UZU)
// UZU expects: A[M,K], B^T[N,K] (transpose_b=true means B is stored as [N,K])
// So we need to pass B as [N,K] to MLX too, then transpose it
...
inline BenchmarkResult benchmark_matmul_f16(
    ...
    // Match UZU weight layout: B is stored as [N, K] and viewed as [K, N] via transpose
