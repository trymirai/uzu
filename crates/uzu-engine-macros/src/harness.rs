...
pub fn define_test_harness(input: TokenStream) -> TokenStream {
    if input.into_iter().next().is_some() {
        return quote! { compile_error!("expected no arguments"); }.into();
    }
        ...
        mod __uzu_test_harness {
            ...
            pub(crate) fn uzu_harness(tests: &[&UzuTest]) {
                ...
                if benchmarks {
                    ...
                    let bench_tests: ::std::vec::Vec<&dyn Fn()> = tests
                        .iter()
                    ...
                    let default_tests: ::std::vec::Vec<&test::TestDescAndFn> = tests
                        .iter()
                        .filter_map(|test| match test {
            ...
            fn enabled(name: &str) -> bool {
                ::std::env::var(name)
                    .is_ok_and(|v| v == "1" || v.eq_ignore_ascii_case("yes") || v.eq_ignore_ascii_case("true"))
            }
        }
    }
    .into()
