...
#[test]
fn malformed_request_body_yields_openai_http_400() {
    let client =
        rocket::local::blocking::Client::tracked(rocket::build().mount("/", rocket::routes![request_body_err_route]))
            .expect("rocket client");
    let response = client.get("/request-body-err").dispatch();
...
#[test]
fn malformed_json_syntax_yields_openai_http_400() {
    let client =
        rocket::local::blocking::Client::tracked(rocket::build().mount("/", rocket::routes![request_body_route]))
            .expect("rocket client");
    let response =
        client.post("/request-body").header(rocket::http::ContentType::JSON).body(r#"{"messages":[}"#).dispatch();
