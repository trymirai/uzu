...
use crate::sys::registry::{IoServiceIterator, dictionary_data, registry_properties};
...
fn cluster_voltage_state_keys(dictionary: &CFDictionary) -> Option<(String, String)> {
    ...
    let mut clusters: Vec<(u8, String)> = data
        .as_chunks::<8>()
        .0
        .iter()
        .map(|cluster| (cluster[1], format!("voltage-states{}-sram", cluster[0])))
        .collect();
