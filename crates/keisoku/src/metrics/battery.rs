...
impl BatteryMetrics {
    ...
    pub(crate) fn read() -> Option<BatteryMetrics> {
        ...
        for index in 0..list.count() {
            ...
            let string = |key: &core::ffi::CStr| {
                dictionary_get::<CFString>(&description, key.to_str().ok()?).map(|s| s.to_string())
            };
            let number = |key: &core::ffi::CStr| {
                dictionary_get::<CFNumber>(&description, key.to_str().ok()?).and_then(|n| n.as_i64())
            };
            let boolean = |key: &core::ffi::CStr| {
                dictionary_get::<CFBoolean>(&description, key.to_str().ok()?).map(|b| b.value())
            };
            ...
            let on_ac_power =
                string(kIOPSPowerSourceStateKey).is_some_and(|state| state.eq_ignore_ascii_case("AC Power"));
