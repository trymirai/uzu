...
def _group_results(
    ...
    """Group results into: combo -> (K, N) -> dispatch_path -> {M: duration_ms}"""
...
def _generate_combo_chart(
    ...
    combo_display = combo.replace("*", "\u00d7").replace("->", " \u2192 ")
    fig.suptitle(
        f"{combo_display} Latency vs Batch Size",
        fontsize=16,
        fontweight="bold",
        y=1.0,
    )
    subtitle_parts = [f"{device}"]
    if source_filename:
        subtitle_parts.append(source_filename)
    subtitle_parts.append("Labels show % change vs batch size 1 for each dispatch path.")
